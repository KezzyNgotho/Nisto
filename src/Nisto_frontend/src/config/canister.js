...
export const LOCAL_CANISTERS = {
  ...
  INTERNET_IDENTITY: 'be2us-64aaa-aaaaa-qaabq-cai', // Local Internet Identity canister ID (will be updated after dfx deploy)
};

// ===== LIVE ENVIRONMENT (Mainnet) =====
export const LIVE_CANISTERS = {
  NISTO_BACKEND: 'tp4se-aiaaa-aaaau-abyha-cai', // Live backend canister ID - DEPLOYED
  NISTO_FRONTEND: 'ti5uq-nqaaa-aaaau-abyhq-cai', // Live frontend canister ID - DEPLOYED
  INTERNET_IDENTITY: 'rdmx6-jaaaa-aaaah-qcaiq-cai', // Live Internet Identity canister ID (mainnet)
};

// ===== ENVIRONMENT SELECTION =====
// Set this to 'local' for development, 'live' for production
// Note: For local testing, you can use mainnet Internet Identity by setting this to 'live'
const ENVIRONMENT = 'live'; // Changed to 'live' to use mainnet Internet Identity for testing
...
export const validateCanisterIds = () => {
  ...
  if (!CANISTERS.NISTO_BACKEND) {
    issues.push('NISTO_BACKEND canister ID not configured');
  }
  
  if (!CANISTERS.NISTO_FRONTEND) {
    issues.push('NISTO_FRONTEND canister ID not configured');
  }
