...
export function AuthProvider({ children }) {
  ...
  const retryInitialization = useCallback(async () => {
    console.log('AuthContext: Retrying backend initialization...');
    setIsLoading(true);
    try {
      if (backend) {
        // First check if dfx is running
        const dfxRunning = await backend.checkDfxConnection();
        console.log('AuthContext: DFX running:', dfxRunning);
        
        if (!dfxRunning) {
          throw new Error('DFX is not running. Please start dfx with: dfx start --clean --background');
        }
        
        // Try to reinitialize the actor
        const actorReinitialized = await backend.reinitializeActor();
        console.log('AuthContext: Actor reinitialized:', actorReinitialized);
        
        const initResult = await backend.init();
        console.log('AuthContext: Retry initialization result:', initResult);
        
        const isInitialized = backend.isInitialized();
        console.log('AuthContext: BackendService initialized after retry:', isInitialized);
        
        const authed = backend.isAuthenticated;
        setIsAuthenticated(authed);
        
        if (authed) {
          await fetchUser(backend);
          await refreshRecoveryMethods(backend);
          await refreshCryptoWallets(backend);
        }
      ...
      console.error('AuthContext: Retry initialization failed:', error);
      throw error;
    } finally {
      setIsLoading(false);
    }
  }, [backend, fetchUser, refreshRecoveryMethods, refreshCryptoWallets]);
