...
export function AuthProvider({ children }) {
  ...
  const fetchUser = useCallback(async (backendService = backend) => {
    ...
    try {
      ...
      // Temporarily disabled auto-username generation to fix Internet Identity
      // Auto-generate a username if missing
      // if (!userData?.displayName) {
      //   try {
      //     const principalText = backendService.principal ? backendService.principal.toString() : '';
      //     const suffix = principalText ? principalText.replace(/[^a-z0-9]/gi, '').slice(0, 8).toLowerCase() : Math.random().toString(36).slice(2, 10);
      //     const generated = `nesto_${suffix}`;
      //     const updateResult = await backendService.updateUser(generated, null);
      //     // Refetch after update
      //     userData = await backendService.getUser();
      //   } catch (_) {
      //     // ignore generation errors, keep original userData
      //   }
      // }
      setUser(userData);
    } catch (e) {
