...
export function AuthProvider({ children }) {
  ...
  // Initialize real-time notification service when backend is available
  useEffect(() => {
    if (backend && user) {
      // Initialize real-time notification service
      if (window.notificationContext) {
        window.notificationContext.initializeRealTimeService(backend);
      }
    }
  }, [backend, user]);
  ...
  const respondToVaultInvitation = useCallback(async (vaultId, response) => {
    if (!backend) throw new Error('BackendService not initialized');
    return backend.respondToVaultInvitation(vaultId, response);
  }, [backend]);
  ...
  const createNotification = useCallback(async (notificationType, title, message, priority, actionUrl, expiresAt) => {
    if (!backend) throw new Error('BackendService not initialized');
    return backend.createNotification(notificationType, title, message, priority, actionUrl, expiresAt);
  }, [backend]);

  const getUserNotifications = useCallback(async (limit = 20, offset = 0) => {
    if (!backend) return [];
    try {
      console.log('AuthContext: Calling backend.getUserNotifications() with limit:', limit, 'offset:', offset);
      const result = await backend.getUserNotifications(limit, offset);
      console.log('AuthContext: Backend returned:', result);
      
      if (result && result.ok !== undefined) {
        console.log('AuthContext: Returning result.ok:', result.ok);
        return result.ok;
      } else if (Array.isArray(result)) {
        console.log('AuthContext: Returning direct array:', result);
        return result;
      } else {
        console.error('AuthContext: Unexpected result format:', result);
        return [];
      }
    } catch (error) {
      console.error('AuthContext: Failed to get user notifications:', error);
      return [];
    }
  }, [backend]);
  ...
  const markNotificationAsRead = useCallback(async (notificationId) => {
    if (!backend) throw new Error('BackendService not initialized');
    return backend.markNotificationAsRead(notificationId);
  }, [backend]);

  const markAllNotificationsAsRead = useCallback(async () => {
    if (!backend) throw new Error('BackendService not initialized');
    return backend.markAllNotificationsAsRead();
  }, [backend]);

  const deleteNotification = useCallback(async (notificationId) => {
    if (!backend) throw new Error('BackendService not initialized');
    return backend.deleteNotification(notificationId);
  }, [backend]);

  const addPushSubscription = useCallback(async (subscriptionData) => {
    if (!backend) throw new Error('BackendService not initialized');
    return backend.addPushSubscription(subscriptionData);
  }, [backend]);

  const removePushSubscription = useCallback(async (subscriptionId) => {
    if (!backend) throw new Error('BackendService not initialized');
    return backend.removePushSubscription(subscriptionId);
  }, [backend]);
  ...
  const updateNotificationPreferences = useCallback(async (
    ...
    if (!backend) throw new Error('BackendService not initialized');
    return backend.updateNotificationPreferences(
      email, push, inApp, transactionNotifications, securityNotifications,
      vaultNotifications, socialNotifications, systemNotifications,
      paymentNotifications, recoveryNotifications, quietHours, timezone
    );
  ...
  const getNotificationPreferences = useCallback(async () => {
    if (!backend) return null;
    try {
      const result = await backend.getNotificationPreferences();
      if (result.ok) {
        return result.ok;
      } else {
        console.error('Failed to get notification preferences:', result.err);
        return null;
      }
    } catch (error) {
  ...
  const sendCrossCanisterNotification = useCallback(async (targetCanisterId, notificationData) => {
    if (!backend) throw new Error('BackendService not initialized');
    return backend.sendCrossCanisterNotification(targetCanisterId, notificationData);
  }, [backend]);

  const getUnreadNotificationCount = useCallback(async () => {
    if (!backend) return 0;
    try {
      console.log('AuthContext: Calling backend.getUnreadNotificationCount()');
      const result = await backend.getUnreadNotificationCount();
      console.log('AuthContext: Backend returned:', result);
      ...
      if (result && result.ok !== undefined) {
        ...
        console.log('AuthContext: Returning direct number:', result);
        return result;
      } else {
        console.error('AuthContext: Unexpected result format:', result);
        return 0;
      ...
      console.error('AuthContext: Failed to get unread notification count:', error);
      return 0;
    }
  }, [backend]);
