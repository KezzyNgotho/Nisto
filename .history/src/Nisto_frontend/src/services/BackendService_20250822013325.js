...
// Import idlFactory - will be loaded dynamically
let idlFactory = null;
...
class BackendService {
  ...
  async loadIdlFactory() {
    if (idlFactory) return idlFactory;
    
    try {
      console.log('BackendService: Loading idlFactory...');
      const module = await import('../../../declarations/Nisto_backend/Nisto_backend.did.js');
      idlFactory = module.idlFactory;
      console.log('BackendService: idlFactory loaded successfully');
      return idlFactory;
    } catch (error) {
      console.warn('BackendService: Could not load idlFactory:', error);
      throw new Error('Failed to load backend interface. Please run: dfx generate');
    }
  }
  ...
  async createActor() {
    try {
      ...
      // Load idlFactory if not already loaded
      const factory = await this.loadIdlFactory();
      ...
      this.actor = Actor.createActor(factory, {
        agent,
  ...
  async ensureActor() {
    try {
      ...
      // Test the actor to make sure it's working
      if (this.actor) {
        try {
          // Try a simple call to test the connection
          await this.actor.userExists();
          console.log('BackendService: Actor connection test successful');
        } catch (testError) {
          console.warn('BackendService: Actor connection test failed, recreating actor:', testError);
          this.actor = null;
          await this.createActor();
        }
      }
