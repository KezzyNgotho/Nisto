...
class MarketDataService {
  ...
  async getCurrencyPrice(currencySymbol) {
    try {
      ...
      const response = await fetch(
        `${this.baseURLs.coingecko}/simple/price?ids=${currencySymbol.toLowerCase()}&vs_currencies=usd&include_24hr_change=true`
      );
      ...
      const searchResponse = await fetch(
        `${this.baseURLs.coingecko}/search?query=${currencySymbol}`
      );
      ...
      if (searchResponse.ok) {
        ...
        if (searchData.coins && searchData.coins.length > 0) {
          ...
          const priceResponse = await fetch(
            `${this.baseURLs.coingecko}/simple/price?ids=${coinId}&vs_currencies=usd&include_24hr_change=true`
          );
