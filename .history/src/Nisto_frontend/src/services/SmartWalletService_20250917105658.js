...
class SmartWalletService {
  ...
  async initialize() {
    ...
    try {
      ...
      const { Nisto_backend } = await import('../../../../src/declarations/Nisto_backend/index.js');
      this.actor = Nisto_backend;
      ...
      try {
        ...
        // If backend test fails, fallback to mock
        console.log('🔄 SmartWalletService: Falling back to mock actor due to backend test failure');
  ...
  async getUserPreferences(userId) {
    ...
    if (!this.actor) {
      console.error('❌ SmartWalletService: Actor not initialized, using fallback');
      // Return default preferences if actor is not available
      return {
        thresholdPercentage: 10,
        timeWindowMinutes: 30,
        autoHedging: true,
  ...
  async getSwapHistory(userId) {
    ...
    if (!this.actor) {
      console.error('❌ SmartWalletService: Actor not initialized, returning empty history');
      return [];
    }
