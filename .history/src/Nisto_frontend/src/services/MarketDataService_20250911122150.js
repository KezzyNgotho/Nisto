    ...
    try {
      ...
      const coinsWithPrices = await Promise.all(
        data.coins?.map(async (coin) => {
          try {
            const price = await this.getCurrencyPrice(coin.id);
            return {
              id: coin.id,
              symbol: coin.symbol.toUpperCase(),
              name: coin.name,
            ...
            console.error(`❌ MarketDataService: Error fetching price for ${coin.id}:`, error);
            return {
              id: coin.id,
              symbol: coin.symbol.toUpperCase(),
              name: coin.name,
      ...
      console.log('✅ MarketDataService: Search results with prices:', {
        totalResults: coinsWithPrices.length,
        sampleResults: coinsWithPrices.slice(0, 3).map(coin => ({
          id: coin.id,
          symbol: coin.symbol,
          name: coin.name,
          price: coin.price
