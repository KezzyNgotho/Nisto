...
class BackendService {
  ...
  async getUSDTtoKESRate() {
    try {
      // Call Binance API directly for real-time rates
      const binanceApiKey = "l7vVjSBprwpm5immi3iMJ8T26V7i55KHQH1yDtE4JDfVfbICPIUmOfOQk5IZLLXQ";
      
      // Get USDT/NGN rate from Binance
      ...
      console.log(`Binance API: 1 USDT = ${usdtNgnRate} NGN = ${usdtKesRate} KES`);
      
      return usdtKesRate;
    } catch (error) {
      console.error('Error fetching USDT to KES rate from Binance:', error);
      
      // Fallback to backend if API fails
      try {
        const actor = await this.ensureActor();
  ...
  // Enhanced Binance API methods
  async getBinanceRates() {
    try {
      const binanceApiKey = "l7vVjSBprwpm5immi3iMJ8T26V7i55KHQH1yDtE4JDfVfbICPIUmOfOQk5IZLLXQ";
      
      // Get multiple currency pairs for better accuracy
