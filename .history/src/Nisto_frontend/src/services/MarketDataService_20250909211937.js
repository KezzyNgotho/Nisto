...
class MarketDataService {
  ...
  // Get all major cryptocurrencies with live data
  async getAllCurrencies() {
    try {
      const url = `${this.baseUrls.coinGecko}/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=50&page=1&sparkline=false&price_change_percentage=24h`;
  ...
  async searchCurrencies(query) {
    try {
      ...
      return data.coins?.map(coin => ({
        ...
        symbol: coin.symbol.toUpperCase(),
        name: coin.name,
        price: coin.market_cap_rank ? await this.getCurrencyPrice(coin.id) : 0,
        change24h: 0,
  ...
  async getTrendingCurrencies() {
    try {
      ...
      return data.coins?.map(coin => ({
        ...
        name: coin.item.name,
        price: await this.getCurrencyPrice(coin.item.id),
        change24h: 0,
