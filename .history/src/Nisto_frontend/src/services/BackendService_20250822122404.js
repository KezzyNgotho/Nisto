...
class BackendService {
  ...
  async ensureActor() {
    try {
      ...
      // Only test the actor if we're in development mode and want to verify connection
      if (this.actor && process.env.NODE_ENV === 'development') {
        try {
          // Try a simple call to test the connection (only in dev mode)
          await this.actor.userExists();
          console.log('BackendService: Actor connection test successful');
        } catch (testError) {
          console.warn('BackendService: Actor connection test failed, but continuing:', testError.message);
          // Don't recreate actor on test failure, just log the warning
        }
      }
