...
class BackendService {
  ...
  async getUSDTtoKESRate() {
    try {
      // For now, return a mock rate. In production, this would call a crypto API
      // You can integrate with Binance, CoinGecko, or KuCoin API here
      const mockRate = 135.50; // 1 USDT = 135.50 KES
      return mockRate;
      ...
      // const kesRate = ngnRate * 0.25; // Approximate NGN to KES conversion
      // return kesRate;
    } catch (error) {
      console.error('Error fetching USDT to KES rate:', error);
  ...
  async processPaybillPayment(paymentData) {
    try {
      ...
      // 2. Backend would call Mpesa API
      // 3. Return transaction details
      
      const mockTransactionId = `TXN_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      const mockMpesaReceipt = `MPESA_${Date.now()}`;
      ...
      // Example real implementation:
      // const actor = await this.ensureActor();
      // return await actor.processPaybillPayment(paymentData);
    } catch (error) {
  ...
  async getPaybillTransactionHistory() {
    try {
      ...
      // Example real implementation:
      // const actor = await this.ensureActor();
      // return await actor.getPaybillTransactionHistory();
    } catch (error) {
