...
class MarketDataService {
  ...
  async getCryptoPrices(currencyIds = ['tether', 'usd-coin', 'dai', 'internet-computer']) {
    try {
      ...
      const proxyUrl = 'https://thingproxy.freeboard.io/fetch/';
      const targetUrl = `${this.baseURLs.coingecko}/simple/price?ids=${currencyIds.join(',')}&vs_currencies=usd&include_24hr_change=true`;
      const response = await fetch(proxyUrl + targetUrl);
  ...
  async getCurrencyPrice(currencySymbol) {
    try {
      ...
      const searchResponse = await fetch(proxyUrl + searchTargetUrl);
      ...
      if (searchResponse.ok) {
        ...
        if (searchData.coins && searchData.coins.length > 0) {
          ...
          const priceResponse = await fetch(proxyUrl + priceTargetUrl);
  ...
  async searchCryptocurrencies(query) {
    try {
      ...
      // Use CORS proxy for localhost development
      const proxyUrl = 'https://thingproxy.freeboard.io/fetch/';
      const targetUrl = `${this.baseURLs.coingecko}/search?query=${encodeURIComponent(query)}`;
  ...
  async getSpecificCryptocurrencies(coinIds = ['pepe', 'maga', 'trump']) {
    try {
      ...
      const proxyUrl = 'https://thingproxy.freeboard.io/fetch/';
      const targetUrl = `${this.baseURLs.coingecko}/coins/markets?vs_currency=usd&ids=${coinIds.join(',')}&order=market_cap_desc&per_page=100&page=1&sparkline=false&price_change_percentage=24h`;
      const response = await fetch(proxyUrl + targetUrl);
