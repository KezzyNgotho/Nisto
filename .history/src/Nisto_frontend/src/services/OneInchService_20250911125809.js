...
class OneInchService {
  ...
  async getSwapQuote(fromToken, toToken, amount, slippage = 1) {
    ...
    try {
      ...
      if (!fromTokenData || !toTokenData) {
        console.log(`⚠️ OneInchService: Token not supported by 1inch: ${fromToken} or ${toToken}`);
        console.log('🔄 OneInchService: Returning mock quote for unsupported tokens');
        
        // Return mock quote for unsupported tokens
  ...
  getMockRate(fromToken, toToken) {
    ...
    if (rate) {
      console.log(`💰 OneInchService: Mock rate for ${rateKey}: ${rate}`);
      return rate;
    }
    
    // Default rate if not found
    console.log(`💰 OneInchService: No mock rate found for ${rateKey}, using default rate: 1`);
    return 1;
  }
