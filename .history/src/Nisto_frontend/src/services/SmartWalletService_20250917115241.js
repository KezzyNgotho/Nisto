...
class SmartWalletService {
  ...
  async initialize() {
    ...
    try {
      ...
      let importedModule;
      try {
        importedModule = await import('../../../../src/declarations/Nisto_backend/index.js');
      } catch (error) {
        console.log('🔍 SmartWalletService: First import path failed, trying alternative...');
        try {
          importedModule = await import('../../../../../src/declarations/Nisto_backend/index.js');
        } catch (error2) {
          console.log('🔍 SmartWalletService: Second import path failed, trying relative...');
          importedModule = await import('../../declarations/Nisto_backend/index.js');
        }
      }
      ...
      } else if (importedModule.createActor && importedModule.canisterId) {
        this.actor = importedModule.createActor(importedModule.canisterId);
        console.log('✅ SmartWalletService: Created new actor using createActor');
      } else {
        throw new Error('Cannot create actor: missing createActor function or canisterId');
      }
