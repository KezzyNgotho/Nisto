...
class SmartWalletService {
  ...
  async saveUserPreferences(userId, preferences) {
    ...
    try {
      // Convert userId to string if it's a Principal object
      const userIdString = typeof userId === 'string' ? userId : userId.toString();
      
      // Ensure all required fields are present with defaults
      const completePreferences = {
        thresholdPercentage: preferences.thresholdPercentage ?? 20,
        timeWindowMinutes: preferences.timeWindowMinutes ?? 60,
        stablecoinPreference: preferences.stablecoinPreference ?? 'USDC',
        autoHedgingEnabled: preferences.autoHedgingEnabled ?? false,
        allocationLimit: preferences.allocationLimit ?? 50,
        intermediaryTokenPreference: preferences.intermediaryTokenPreference ?? 'USDC',
        preferredSwapProvider: preferences.preferredSwapProvider ?? 'auto',
        ...preferences // Override with any provided values
      };
      
      // Convert optional fields to proper Motoko optional format
      const formattedPreferences = {
        ...completePreferences,
        intermediaryTokenPreference: completePreferences.intermediaryTokenPreference ? 
          { Some: completePreferences.intermediaryTokenPreference } : 
          { None: null },
        preferredSwapProvider: completePreferences.preferredSwapProvider ? 
          { Some: completePreferences.preferredSwapProvider } : 
          { None: null }
      };
      
      console.log('🔍 SmartWalletService: Calling saveAIUserPreferences with userId:', userIdString);
      console.log('🔍 SmartWalletService: Complete preferences:', completePreferences);
      console.log('🔍 SmartWalletService: Formatted preferences:', formattedPreferences);
      await this.actor.saveAIUserPreferences(userIdString, formattedPreferences);
  ...
  async getUserPreferences(userId) {
    ...
    try {
      // Convert userId to string if it's a Principal object
      const userIdString = typeof userId === 'string' ? userId : userId.toString();
      console.log('🔍 SmartWalletService: Calling getAIUserPreferences with userId:', userIdString);
      const preferences = await this.actor.getAIUserPreferences(userIdString);
      console.log('📋 SmartWalletService: Retrieved user preferences:', preferences);
      
      // Convert optional fields from Motoko format to JavaScript format
      if (preferences) {
        const formattedPreferences = {
          ...preferences,
          intermediaryTokenPreference: preferences.intermediaryTokenPreference?.Some || null,
          preferredSwapProvider: preferences.preferredSwapProvider?.Some || null
        };
        console.log('📋 SmartWalletService: Formatted preferences for frontend:', formattedPreferences);
  ...
  async updateUserPreferences(userId, preferences) {
    ...
    try {
      ...
      const formattedPreferences = {
        ...preferences,
        intermediaryTokenPreference: preferences.intermediaryTokenPreference ? 
          { Some: preferences.intermediaryTokenPreference } : 
          { None: null },
        preferredSwapProvider: preferences.preferredSwapProvider ? 
          { Some: preferences.preferredSwapProvider } : 
          { None: null }
      };
      
      console.log('🔍 SmartWalletService: Calling updateAIUserPreferences with userId:', userIdString);
      console.log('🔍 SmartWalletService: Formatted preferences:', formattedPreferences);
      await this.actor.updateAIUserPreferences(userIdString, formattedPreferences);
  ...
  async addSwapRecord(userId, swapData) {
    ...
    try {
      // Convert userId to string if it's a Principal object
      const userIdString = typeof userId === 'string' ? userId : userId.toString();
      console.log('🔍 SmartWalletService: Calling addAISwapRecord with userId:', userIdString);
  ...
  async getSwapHistory(userId) {
    ...
    try {
      // Convert userId to string if it's a Principal object
      const userIdString = typeof userId === 'string' ? userId : userId.toString();
      console.log('🔍 SmartWalletService: Calling getAISwapHistory with userId:', userIdString);
      const history = await this.actor.getAISwapHistory(userIdString);
  ...
  async analyzePortfolioAndSwap(userId, currentPortfolio, marketData) {
    ...
    try {
      ...
      // Convert userId to string if it's a Principal object
      const userIdString = typeof userId === 'string' ? userId : userId.toString();
      console.log('🔍 SmartWalletService: Calling analyzePortfolioAndSwap with userId:', userIdString);
      const analysis = await this.actor.analyzePortfolioAndSwap(userIdString, portfolioAssets, currencies);
