...
class SwapBackendService {
  ...
  async initialize() {
    ...
    try {
      ...
      this.agent = new HttpAgent({
        host: import.meta.env.MODE === 'development' 
          ? 'http://localhost:4943' 
          : 'https://ic0.app'
      });
      ...
      // Create actor
      const canisterId = import.meta.env.MODE === 'development'
        ? import.meta.env.VITE_BACKEND_CANISTER_ID || 'rrkah-fqaaa-aaaah-qcvmq-cai'
  ...
  async getStats() {
    await this.initialize();
    try {
      return await this.actor.getSwapStats();
    } catch (error) {
