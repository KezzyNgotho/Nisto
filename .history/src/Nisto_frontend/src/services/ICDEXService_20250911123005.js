...
class ICDEXService {
  ...
  async fetchSonicTokens() {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Sonic API error: ${response.status} ${response.statusText}`);
      }

      const data = await response.json();
      console.log('✅ ICDEXService: Sonic tokens fetched successfully:', data.tokens?.length || 0);
      return data.tokens || [];
  ...
  async getSonicPools(tokenA, tokenB) {
    try {
      ...
      if (!response.ok) {
        console.log(`⚠️ ICDEXService: Sonic pools API returned ${response.status} ${response.statusText}`);
        return [];
      }

      const data = await response.json();
      console.log(`✅ ICDEXService: Sonic pools fetched for ${tokenA}/${tokenB}:`, data.pools?.length || 0);
      return data.pools || [];
