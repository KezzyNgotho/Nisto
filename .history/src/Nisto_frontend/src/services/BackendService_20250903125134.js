...
import { CANISTERS, ENV_INFO, validateCanisterIds } from '../config/canister';

import { idlFactory } from '../../../declarations/Nisto_backend/Nisto_backend.did.js';

// Use centralized canister configuration
const getCanisterId = () => {
  // Validate configuration first
  if (!validateCanisterIds()) {
    console.warn('BackendService: Canister configuration validation failed, using fallback');
  }
  
  const canisterId = CANISTERS.NISTO_BACKEND;
  console.log(`BackendService: Using ${ENV_INFO.current} canister ID: ${canisterId}`);
  return canisterId;
};
...
const getInternetIdentityCanisterId = () => {
  if (ENV_INFO.isLive) {
    ...
    return CANISTERS.INTERNET_IDENTITY;
  }
  ...
  console.log('BackendService: Using hardcoded local Internet Identity canister ID:', CANISTERS.INTERNET_IDENTITY);
  return CANISTERS.INTERNET_IDENTITY;
};
...
console.log('Network:', ENV_INFO.network);
...
class BackendService {
  ...
  async createActor() {
    try {
      ...
      console.log('BackendService: Host:', ENV_INFO.isLocal ? 'http://localhost:4943' : 'https://ic0.app');
      console.log('BackendService: isLocal value:', ENV_INFO.isLocal);
      
      const agent = new HttpAgent({ 
        identity: this.identity, // Can be null for unauthenticated operations
        host: ENV_INFO.isLocal ? 'http://localhost:4943' : 'https://ic0.app'
      });
  ...
  async login() {
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        const identityProvider = ENV_INFO.isLocal 
        ...
        console.log('isLocal value:', ENV_INFO.isLocal);
        ...
        this.authClient.login({
          ...
          ...(ENV_INFO.isLocal ? { derivationOrigin: window.location.origin } : {}),
    ...
    try {
      ...
      if (typeof userId === 'string') {
        try {
          ...
          throw new Error('Invalid principal format: ' + userId);
        }
      } else {
        principal = userId;
      }

      const result = await this.actor.inviteToVault(vaultId, principal, role);
      if (result.ok) {
        return { ok: { invitationId: result.ok.id, message: "Invitation sent successfully" } };
      } else {
        throw new Error(result.err);
      }
    } catch (error) {
  ...
  async getUserNotifications(limit = 20, offset = 0) {
    try {
      ...
      console.log('BackendService: Safe result structure:', safeResult);
      return safeResult;
    } catch (error) {
