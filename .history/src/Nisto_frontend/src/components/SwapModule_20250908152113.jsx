...
const SwapModule = ({ onBack }) => {
  ...
  const loadCurrencies = async () => {
    ...
    try {
      ...
      // Also load specific cryptocurrencies like PEPE, TRUMP, etc.
      const specificCryptos = await marketDataService.getSpecificCryptocurrencies([
        'pepe', 'maga', 'trump', 'doge', 'shiba-inu', 'floki', 'bonk'
      ]);
      
      // Combine all cryptocurrencies
      const combinedCryptos = [...allCryptos, ...specificCryptos];
      
      // Remove duplicates based on ID
      const uniqueCryptos = combinedCryptos.filter((crypto, index, self) => 
      ...
      const transformedCurrencies = uniqueCryptos.map(crypto => {
        ...
        const getIcon = (symbol) => {
          switch (symbol.toLowerCase()) {
            ...
            case 'pepe': return FiZap;
            case 'trump': return FiStar;
            case 'maga': return FiStar;
            case 'doge': return FiZap;
            case 'shib': return FiZap;
            case 'floki': return FiZap;
            case 'bonk': return FiZap;
        ...
        const getColor = (symbol) => {
          switch (symbol.toLowerCase()) {
            ...
            case 'pepe': return theme.colors.success || '#00ff00';
            case 'trump': return theme.colors.warning || '#ff6b35';
            case 'maga': return theme.colors.error || '#ff0000';
            case 'doge': return theme.colors.warning || '#ffd700';
            case 'shib': return theme.colors.warning || '#ffa500';
            case 'floki': return theme.colors.info || '#00bfff';
            case 'bonk': return theme.colors.primary || '#8b5cf6';
      ...
      // Combine all currencies (now showing many more)
      const allCurrencies = [nstToken, ...transformedCurrencies, kesCurrency];
              ...
              {currenciesLoading ? (
                <div style={{ 
                  ...
                  <div style={{ fontSize: '0.8rem', marginTop: '0.25rem', opacity: 0.7 }}>
                    Including PEPE, TRUMP, DOGE, and more!
                  </div>
                </div>
              ) : (
