...
const PayBills = () => {
  ...
  useEffect(() => {
    const loadBills = async () => {
      ...
      setAvailableBills(defaultBills);

      // Try to load from backend if service exists
      if (backendService && typeof backendService.getPaymentServices === 'function') {
        try {
          const services = await backendService.getPaymentServices();
          if (services && services.length > 0) {
            ...
            setAvailableBills(convertedServices);
          }
        } catch (error) {
          console.error('Error loading payment services from backend:', error);
          // Keep using default bills
        }
      }
    };
  ...
  const getIconFromName = (iconName) => {
    switch (iconName) {
      ...
      case 'FiShoppingCart': return FiShoppingCart;
      case 'FiSend': return FiSend;
      case 'FiSmartphone': return FiSmartphone;
      case 'FiArrowRight': return FiArrowRight;
  ...
  const handlePayment = async () => {
    ...
    // Additional validation for buy airtime
    if (selectedBill.id === 'buy-airtime' && !recipientPhone) {
      setError('Please fill in phone number');
      return;
    }

    // Additional validation for withdraw
    if (selectedBill.id === 'withdraw' && (!accountNumber || !recipientName)) {
      setError('Please fill in bank account and account holder name');
      return;
    }
      ...
      <div style={{ marginBottom: '2rem' }}>
              ...
              <div
                ...
                >
                  ...
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: '0.75rem', fontWeight: '600', color: '#1e293b', marginBottom: '0.125rem' }}>
                      {bill.name}
                    </div>
        ...
        {/* Wallet Selection */}
         ...
         {selectedBill && selectedBill.id === 'buy-goods' && (
          ...
          {/* Buy Airtime Fields */}
          {selectedBill && selectedBill.id === 'buy-airtime' && (
            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ 
                display: 'block', 
                marginBottom: '0.5rem', 
          ...
          {/* Withdraw Fields */}
          {selectedBill && selectedBill.id === 'withdraw' && (
            <div style={{ marginBottom: '1.5rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  <label style={{ 
