...
const TokenDashboard = () => {
  ...
  useEffect(() => {
    if (backendService && auth?.user) {
      loadTokenData();
    }
  }, [backendService, auth?.user]);

  // Auto-refresh every 30 seconds if enabled
  useEffect(() => {
    if (!autoRefresh || !backendService || !auth?.user) return;
  ...
  }, [autoRefresh, backendService, auth?.user]);

  const loadTokenData = useCallback(async (silent = false) => {
    if (!backendService || !auth?.user) return;
    ...
    try {
      ...
      console.log('Loading token data for user:', auth.user.id);
      
      const [metadata, userBalance, userTotalBalance, userStakingInfo, history] = await Promise.all([
        backendService.getTokenMetadata(),
        backendService.balanceOf(auth.user.id),
        backendService.getTotalBalance(auth.user.id),
        backendService.getStakingInfo(auth.user.id),
