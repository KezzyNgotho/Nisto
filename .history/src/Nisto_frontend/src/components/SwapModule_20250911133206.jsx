...
import swapBackendService from '../services/swapBackend';
...
const SwapModule = ({ onBack }) => {
  ...
  // Set default currencies when loaded (only once)
  useEffect(() => {
    if (currencies.length > 0 && !swapForm.fromCurrency && !swapForm.toCurrency) {
      ...
      console.log('Setting default currencies:', firstCurrency.id, secondCurrency.id);
  ...
  }, [currencies.length]); // Only depend on currencies.length, not the entire currencies array
  ...
  const loadCurrencies = async () => {
    console.log('🔄 SwapModule: Loading currencies...');
    setCurrenciesLoading(true);
    try {
      // Load currencies from MarketDataService (frontend API calls)
      console.log('📡 SwapModule: Calling MarketDataService.getAllCurrencies()...');
      const allCryptos = await MarketDataService.getAllCurrencies();
      console.log('✅ SwapModule: Received currencies from MarketDataService:', {
        totalCurrencies: allCryptos.length,
        sampleCurrencies: allCryptos.slice(0, 3).map(crypto => ({
          id: crypto.id,
  ...
  const loadSwapData = async () => {
    ...
    try {
      ...
      console.log('🔄 SwapModule: Loading swap data...');
      
      // Note: getMarketOverview doesn't exist in MarketDataService, so we'll skip it
      // and just load currencies which already have market data
      console.log('✅ SwapModule: Swap data loaded successfully');

      // Update currencies with real-time data from API
      setCurrencies(prev => prev.map(currency => {
        // Keep existing currency data - prices are already loaded from API
        return currency;
      }));

      // Update swap form with new rates
      console.log('🔄 SwapModule: Calculating exchange rate...');
      const newRate = await calculateRate(swapForm.fromCurrency, swapForm.toCurrency);
  ...
  const handleFromCurrencyChange = async (currencyId) => {
    console.log('From currency changing to:', currencyId);
    const currency = currencies.find(c => c.id === currencyId);
    if (currency) {
      try {
        // Don't clear amounts if user is just changing currency
        setSwapForm(prev => ({
          ...prev,
  ...
  const handleToCurrencyChange = async (currencyId) => {
    console.log('To currency changing to:', currencyId);
    const currency = currencies.find(c => c.id === currencyId);
    if (currency) {
      try {
        // Don't clear amounts if user is just changing currency
  ...
  const calculateRate = async (fromCurrency, toCurrency) => {
    console.log(`💱 SwapModule: Calculating rate for ${fromCurrency}/${toCurrency}...`);
    
    try {
      // Use CoinGecko for ALL tokens (supports ICP, Bitcoin, Ethereum, etc.)
      console.log(`📡 SwapModule: Using CoinGecko for universal token support...`);
      const rate = await MarketDataService.getExchangeRate(fromCurrency, toCurrency);
      
      if (rate > 0) {
        console.log(`✅ SwapModule: CoinGecko rate found: ${rate}`);
        return rate;
      }
      
      // If CoinGecko doesn't have the pair, try reverse calculation
      console.log(`🔄 SwapModule: Trying reverse calculation...`);
      const reverseRate = await MarketDataService.getExchangeRate(toCurrency, fromCurrency);
      if (reverseRate > 0) {
        const calculatedRate = 1 / reverseRate;
        console.log(`✅ SwapModule: Reverse rate calculated: ${calculatedRate}`);
        return calculatedRate;
      }
      
      throw new Error(`No exchange rate found for ${fromCurrency}/${toCurrency}`);
    } catch (error) {
      console.error('❌ SwapModule: Error calculating rate:', error);
      throw error; // Don't fallback to hardcoded values, let the error propagate
    }
  };

  const handleFromAmountChange = async (amount) => {
    console.log('From amount changing to:', amount);
    
    // Always update the from amount first
    setSwapForm(prev => ({
      ...prev,
      fromAmount: amount
    }));
    ...
    if (!swapForm.fromCurrency || !swapForm.toCurrency) {
      console.log('Missing currencies for calculation');
      return;
    }
  ...
  const handleToAmountChange = async (amount) => {
    console.log('To amount changing to:', amount);
    
    // Always update the to amount first
    setSwapForm(prev => ({
      ...prev,
      toAmount: amount
    }));
  ...
  const handleSwap = async () => {
    ...
    try {
      // Execute swap using CoinGecko-based simulation (supports ALL tokens)
      const swapRequest = {
        ...
        userAddress: user?.principal || 'anonymous'
      };

      console.log('🔄 SwapModule: Executing swap with CoinGecko-based simulation:', swapRequest);
      
      // Simulate swap using CoinGecko rates
      const rate = await calculateRate(swapForm.fromCurrency, swapForm.toCurrency);
      const expectedOutput = swapForm.fromAmount * rate;
      const slippageAmount = expectedOutput * (swapForm.slippage / 100);
      const finalOutput = expectedOutput - slippageAmount;
      
      const swapResult = {
        success: true,
        fromToken: swapForm.fromCurrency,
        toToken: swapForm.toCurrency,
        fromAmount: parseFloat(swapForm.fromAmount),
        toAmount: finalOutput,
        rate: rate,
        gasEstimate: 0.001, // Simulated gas fee
        slippage: swapForm.slippage,
        minimumReceived: finalOutput,
        timestamp: Date.now()
      };
      
      console.log('✅ SwapModule: CoinGecko-based swap result:', swapResult);
      
      if (swapResult.success) {
        setSwapStatus('success');
        
        const swapRecord = {
          id: swapResult.transactionData?.tx?.hash || Date.now().toString(),
          from: `${swapForm.fromAmount} ${swapForm.fromCurrency}`,
          to: `${swapResult.toAmount} ${swapForm.toCurrency}`,
          rate: swapResult.toAmount / swapForm.fromAmount,
          timestamp: new Date().toISOString(),
          status: 'completed',
          gasFee: swapResult.gasEstimate || 0,
          priceImpact: 0.1 // Default price impact
        };
        
        setSwapHistory(prev => [swapRecord, ...prev]);
        showToast(`Successfully swapped ${swapForm.fromAmount} ${swapForm.fromCurrency} to ${swapResult.toAmount} ${swapForm.toCurrency}`, 'success');
      ...
      console.error('❌ SwapModule: Swap error:', error);
      setSwapStatus('error');
      showToast(`Failed to process swap: ${error.message}`, 'error');
  ...
  const calculateGasFee = (fromCurrency, toCurrency, amount) => {
    ...
    // Check if currencies are stablecoins or fiat
    ...
    const isFiat = (currency) => {
      const fiatCurrencies = ['KES', 'USD', 'EUR', 'GBP', 'JPY'];
  ...
  const shouldShowGasFeeProminently = () => {
    ...
    return swapForm.gasFeeUSD > 0.05 || // Show if gas > $0.05
           isFiat(swapForm.fromCurrency) || // Always show for fiat swaps
           isFiat(swapForm.toCurrency);
  };
  ...
  const performSearch = async (searchTerm) => {
    ...
    setIsSearching(true);
    try {
      // Search using MarketDataService
      const searchData = await MarketDataService.searchCurrencies(searchTerm);
      
      if (searchData && searchData.length > 0) {
        // Transform search results to our currency format
        const transformedResults = searchData.map(currency => {
          ...
          return {
            ...
            icon: getIconFromString(currency.icon),
            ...
            <div className="swap-stats">
              <div className="stat-item">
                ...
                <div style={{ fontSize: '0.75rem', color: theme.colors.text.secondary, marginTop: '0.25rem' }}>
                  {swapForm.fromCurrency} Price: {formatPrice(getCurrencyPrice(swapForm.fromCurrency))} | {swapForm.toCurrency} Price: {formatPrice(getCurrencyPrice(swapForm.toCurrency))}
                </div>
              </div>
              <div className="stat-item">
                ...
                <div className="swap-section">
                  ...
                  <div className="currency-selector">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        console.log('From dropdown changed to:', e.target.value);
                        handleFromCurrencyChange(e.target.value);
                      }}
                      style={{ 
                        color: theme.colors.text.primary
                      }}
                ...
                <div className="swap-section">
                  ...
                  <div className="currency-selector">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        console.log('To dropdown changed to:', e.target.value);
                        handleToCurrencyChange(e.target.value);
                      }}
                      style={{ 
                        color: theme.colors.text.primary
                      }}
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem' }}>
                        ...
                        <span style={{ color: theme.colors.text.primary }}>
                          {(() => {
                            ...
                            if (isFiat(swapForm.fromCurrency) || isFiat(swapForm.toCurrency)) {
                              return 'Fiat Bridge';
                            } else {
                              return 'Cross-Chain';
                            }
