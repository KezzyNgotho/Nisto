...
import SmartWalletService from '../services/SmartWalletService.js';
import { 
  FiArrowLeft,
  FiSettings,
  FiRefreshCw,
...
const SwapModule = ({ onBack }) => {
  ...
  const [smartWalletService] = useState(() => {
    try {
      return new SmartWalletService();
    } catch (error) {
      console.error('Failed to initialize SmartWalletService:', error);
      return null;
    }
  });
          ...
          <div className="swap-actions">
            <button 
              ...
              onClick={() => {
                ...
                window.dispatchEvent(new CustomEvent('refreshSwapData'));
              }}
            >
