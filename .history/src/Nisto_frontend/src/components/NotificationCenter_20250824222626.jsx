    ...
    try {
      ...
      if (result && Array.isArray(result)) {
        ...
        const vaultInvitations = result.filter(n => 
          ...
          n.message?.includes('invited to join vault')
        );
        console.log('Found vault invitations:', vaultInvitations);
        
        // TEMPORARY: Add a test vault invitation for debugging
        if (vaultInvitations.length === 0) {
          console.log('No vault invitations found, adding test notification');
          const testNotification = {
            id: 'test-vault-invitation',
            title: 'Vault Invitation',
            message: 'You\'ve been invited to join vault: TEST VAULT as admin. Click to accept or decline.',
        ...
        <div style={{
          ...
          {filteredAndSortedNotifications.length === 0 ? (
            ...
            filteredAndSortedNotifications.map((notification) => {
              ...
              if (notification.title?.includes('invited to join vault') || 
              ...
              // Fallback: If the message contains vault invitation text, treat it as a vault invitation
              if (notification.message && notification.message.includes('invited to join vault')) {
                console.log('Fallback: Treating as vault invitation based on message:', notification.id);
                return (
                  <VaultInvitation
                    key={notification.id}
                    notification={notification}
              ...
              // TEMPORARY: Force render VaultInvitation for testing
              if (notification.message && notification.message.includes('invited to join vault')) {
                console.log('FORCED: Rendering VaultInvitation for testing:', notification.id);
                return (
                  <VaultInvitation
                    key={notification.id}
                    notification={notification}
