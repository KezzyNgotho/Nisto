...
export default function VaultInvitations() {
  ...
  const { respondToVaultInvitation, getUserNotifications } = useAuth();
  ...
  const handleInvitationResponse = async (invitation, response) => {
    ...
    try {
      await respondToVaultInvitation(invitation.vaultId, response);
      ...
      showToast({
        ...
        message: response 
          ? `You have joined ${invitation.vaultName}` 
          : `You have declined the invitation to ${invitation.vaultName}`
      });
      
      // Reload vaults list if needed
      // You might want to trigger a vault list refresh here
