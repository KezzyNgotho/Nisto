...
export default function GroupVaults() {
  try {
    ...
    // Safety check for error color structure
    if (!theme.colors.error || !theme.colors.error.hex) {
      console.error('Error color not properly defined in theme');
      return <div>Theme configuration error</div>;
    }
  const { 
    getUserGroupVaults, 
    createGroupVault, 
  ...
  const [vaultView, setVaultView] = useState('my-vaults'); // 'my-vaults', 'public-vaults', or 'invitations'
  ...
  }, [isAuthenticated, login, getUserGroupVaults]);

  // Get pending invitations count
  const getPendingInvitationsCount = useCallback(async () => {
    try {
      ...
      setPendingInvitationsCount(vaultInvitations.length);
    } catch (error) {
  ...
  // Listen for vault refresh events from invitations
  useEffect(() => {
    const handleVaultRefresh = () => {
      console.log('Vault refresh triggered from invitation response');
      fetchVaults();
    };

    const handleInvitationsCountRefresh = () => {
      console.log('Invitations count refresh triggered');
      getPendingInvitationsCount();
    };

    window.addEventListener('vaults-refresh-needed', handleVaultRefresh);
    window.addEventListener('invitations-count-refresh', handleInvitationsCountRefresh);
    
    return () => {
      window.removeEventListener('vaults-refresh-needed', handleVaultRefresh);
      window.removeEventListener('invitations-count-refresh', handleInvitationsCountRefresh);
    };
  }, [fetchVaults, getPendingInvitationsCount]);

  // Load pending invitations count on mount and when vaults refresh
  useEffect(() => {
    if (isAuthenticated) {
      getPendingInvitationsCount();
    }
  }, [isAuthenticated, getPendingInvitationsCount]);
  ...
  const handleEdit = async (e) => {
    ...
    setError(null);
    try {
      const result = await editGroupVault(
        selectedVault.id,
        editForm.name,
        editForm.description,
        editForm.currency,
        editForm.targetAmount ? parseFloat(editForm.targetAmount) : null,
        editForm.isPublic
      );
      
      if (result && result.ok) {
        setShowEdit(false);
        setEditForm({ name: '', description: '', currency: '', targetAmount: '', isPublic: false });
        showToast({ message: 'Vault updated successfully!', type: 'success', icon: <FiCheck /> });
        ...
        setError(result?.err || 'Failed to update vault');
        showToast({ message: result?.err || 'Failed to update vault', type: 'error', icon: <FiDollarSign /> });
      }
    } catch (err) {
      console.error('Edit vault error:', err);
      setError('Failed to update vault');
      showToast({ message: 'Failed to update vault', type: 'error', icon: <FiDollarSign /> });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDeposit = async () => {
    ...
    setError(null);
    try {
      const result = await depositToVault(selectedVault.id, parseFloat(depositAmount));
      ...
      if (result && result.ok) {
        ...
        showToast({ message: result?.err || 'Failed to deposit', type: 'error', icon: <FiDollarSign /> });
      }
    } catch (err) {
      console.error('Deposit error:', err);
      setError('Failed to deposit');
      showToast({ message: 'Failed to deposit', type: 'error', icon: <FiDollarSign /> });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleWithdraw = async () => {
    ...
    setError(null);
    try {
      const result = await withdrawFromVault(selectedVault.id, parseFloat(withdrawAmount));
      ...
      if (result && result.ok) {
        ...
        showToast({ message: result?.err || 'Failed to withdraw', type: 'error', icon: <FiDollarSign /> });
      }
    } catch (err) {
      console.error('Withdraw error:', err);
      setError('Failed to withdraw');
      showToast({ message: 'Failed to withdraw', type: 'error', icon: <FiDollarSign /> });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div style={{ width: '100%', padding: '0', margin: 0 }}>
      ...
      <div style={{ 
        ...
        <button
          onClick={() => setVaultView('invitations')}
          style={{
            flex: 1,
            padding: '1rem 1.5rem',
            border: 'none',
            borderRadius: '0.75rem',
            background: vaultView === 'invitations' ? theme.colors.primary : 'transparent',
            color: vaultView === 'invitations' ? theme.colors.white : theme.colors.text.secondary,
            fontSize: '0.875rem',
            fontWeight: 600,
            cursor: 'pointer',
            transition: 'all 0.2s',
      ...
      {/* Show Public Vaults, Invitations, or My Vaults based on selected tab */}
      {vaultView === 'public-vaults' ? (
        <PublicVaults />
      ) : vaultView === 'invitations' ? (
        <VaultInvitations />
              ...
              <div style={{
                  ...
                  <div style={{ 
                    ...
                    <button 
                      ...
                      onMouseEnter={(e) => {
                        e.target.style.transform = 'translateY(-2px)';
                        e.target.style.boxShadow = `0 8px 24px ${theme.colors.success}40`;
                      }}
                      onMouseLeave={(e) => {
                        e.target.style.transform = 'translateY(0)';
                        e.target.style.boxShadow = 'none';
                    ...
                    <button 
                      ...
                      onMouseEnter={(e) => {
                        e.target.style.transform = 'translateY(-2px)';
                        e.target.style.boxShadow = `0 8px 24px ${theme.colors.warning}40`;
                      }}
                      onMouseLeave={(e) => {
                        e.target.style.transform = 'translateY(0)';
                        e.target.style.boxShadow = 'none';
                    ...
                    <button 
                      ...
                      onClick={(e) => { 
                        e.stopPropagation(); 
                        handleEditOpen(vault); 
                      }}
                      onMouseEnter={(e) => {
                        e.target.style.transform = 'translateY(-2px)';
                        e.target.style.boxShadow = `0 8px 24px ${theme.colors.shadow.light}`;
                        e.target.style.borderColor = theme.colors.primary;
          ...
          <form onSubmit={handleCreate} style={{
            ...
            {error && (
              <div style={{
                ...
                padding: '0.625rem 0.75rem',
                borderRadius: '0.5rem',
                border: `1px solid ${theme.colors.error.hex}`,
                fontSize: '0.8rem',
                marginTop: '1rem'
              }}>
                {error}
        ...
        <div style={{
          ...
          <form onSubmit={handleEdit} style={{
            ...
            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
              ...
              <h2 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 700, color: theme.colors.text.primary }}>Edit Vault</h2>
              <p style={{ margin: '0.25rem 0 0 0', color: theme.colors.text.secondary, fontSize: '0.8rem' }}>Update vault settings and information</p>
            </div>
            
            <div style={{ display: 'grid', gap: '1rem' }}>
              {/* First Row - Name and Currency */}
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1rem' }}>
              <div>
                  <label style={{ display: 'block', marginBottom: '0.375rem', fontWeight: 600, color: theme.colors.text.secondary, fontSize: '0.75rem' }}>Vault Name *</label>
                  <input 
              ...
              <div>
                  ...
                  <input 
                    ...
                    onChange={e => setEditForm(v => ({ ...v, currency: e.target.value }))} 
                  style={{
                    width: '100%',
                      padding: '0.625rem 0.75rem',
                      border: `1px solid ${theme.colors.border.primary}`,
              ...
              <div style={{
                ...
                <input 
                  ...
                  onChange={e => setEditForm(v => ({ ...v, isPublic: e.target.checked }))}
                  style={{ width: 16, height: 16 }}
                />
                <div>
                  <div style={{ fontWeight: 600, color: theme.colors.primary, fontSize: '0.8rem' }}>Public Vault</div>
              ...
              >
                  {submitting ? 'Updating...' : 'Update Vault'}
              </button>
            </div>
            </div>
          </form>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
              ...
              <h2 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 700, color: theme.colors.text.primary }}>Deposit to Vault</h2>
              <p style={{ margin: '0.25rem 0 0 0', color: theme.colors.text.secondary, fontSize: '0.8rem' }}>Add funds to {selectedVault.name}</p>
            </div>
            
            <div style={{ display: 'grid', gap: '1rem' }}>
              <div>
                <label style={{ display: 'block', marginBottom: '0.375rem', fontWeight: 600, color: theme.colors.text.secondary, fontSize: '0.75rem' }}>Amount *</label>
                <input 
                  type="number" 
                  placeholder="0.00" 
                  value={depositAmount} 
              ...
              <button 
                ...
                style={{
                  ...
                  border: 'none',
                    borderRadius: '0.5rem',
                    background: submitting ? theme.colors.text.secondary : theme.colors.success,
                  color: 'white',
                    fontSize: '0.875rem',
                  fontWeight: 600,
                    cursor: submitting ? 'not-allowed' : 'pointer',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
              ...
              <h2 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 700, color: theme.colors.text.primary }}>Withdraw from Vault</h2>
              <p style={{ margin: '0.25rem 0 0 0', color: theme.colors.text.secondary, fontSize: '0.8rem' }}>Remove funds from {selectedVault.name}</p>
            </div>
        ...
        <div style={{
              ...
              <button 
                ...
                style={{
                  ...
                  border: 'none',
                    borderRadius: '0.5rem',
                    background: submitting ? theme.colors.text.secondary : theme.colors.warning,
                  color: 'white',
                    fontSize: '0.875rem',
                  fontWeight: 600,
                  cursor: submitting ? 'not-allowed' : 'pointer',
    ...
    console.error('Error in GroupVaults component:', error);
    return (
      <div style={{ 
        padding: '2rem', 
        textAlign: 'center', 
