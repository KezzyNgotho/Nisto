...
const PayBills = ({ auth, backendService }) => {
  ...
  const calculateWalletCurrencyAmount = (kesAmount, currency) => {
    ...
    // Then convert USDT to wallet currency
    switch (currency) {
      case 'BTC':
        // Get BTC/USDT rate (approximate)
        const btcRate = 45000; // This should come from API
        return (usdtAmount / btcRate).toFixed(8);
      case 'ETH':
        // Get ETH/USDT rate (approximate)
        const ethRate = 3000; // This should come from API
         ...
         <div className="form-group">
           ...
           <div className="wallets-grid">
             {userWallets.map((wallet) => (
               <div
                 key={wallet.id}
                 className={`wallet-option ${selectedWallet?.id === wallet.id ? 'selected' : ''}`}
               ...
               >
                 ...
                 <div className="wallet-info">
                   ...
                   <span className="wallet-address">{wallet.address.slice(0, 8)}...{wallet.address.slice(-8)}</span>
                 </div>
                 {selectedWallet?.id === wallet.id && (
                   <div className="wallet-selected">
                     <FiCheckCircle />
                   </div>
                 )}
               </div>
         ...
         <div className="form-group">
           ...
           <div className="amount-input-group">
             ...
             <div className="amount-conversion">
               <FiArrowRight />
               <span>{usdtAmount ? `${usdtAmount} USDT` : 'Calculating...'}</span>
             </div>
           </div>
         </div>

         {/* Wallet Currency Amount Display */}
         {selectedWallet && walletCurrencyAmount && (
           <div className="form-group">
             <label>Amount in {selectedWallet.currency}</label>
             <div className="wallet-amount-display">
               <span className="wallet-amount">{walletCurrencyAmount} {selectedWallet.currency}</span>
               <span className="wallet-balance">Available: {selectedWallet.balance} {selectedWallet.currency}</span>
             </div>
           </div>
         )}
         ...
         >
          {loading ? (
            <>
             ...
             <>
               <FiSmartphone />
               Pay {kesAmount ? `${kesAmount} KES` : 'Bill'} with {selectedWallet?.currency}
             </>
           )}
        </button>
