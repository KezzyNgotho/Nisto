...
import ICDEXService from '../services/ICDEXService';
import { 
  FiRotateCcw,
  FiRefreshCw,
  FiTrendingUp,
...
const SwapModule = ({ onBack }) => {
  ...
  const calculateRate = async (fromCurrency, toCurrency) => {
    ...
    try {
      // Use IC DEX for real-time rates from actual liquidity pools
      console.log(`🦄 SwapModule: Trying IC DEX for real-time price...`);
      const rate = await ICDEXService.getRealTimePrice(fromCurrency, toCurrency);
      
      if (rate > 0) {
        console.log(`✅ SwapModule: IC DEX rate found: ${rate}`);
        return rate;
      }
      
      // Fallback to MarketDataService if IC DEX doesn't have the pair
      console.log(`📡 SwapModule: IC DEX no rate, trying MarketDataService...`);
  ...
  const handleSwap = async () => {
    ...
    try {
      // Execute swap using IC DEX service
      const swapRequest = {
        fromToken: swapForm.fromCurrency,
        toToken: swapForm.toCurrency,
        amount: parseFloat(swapForm.fromAmount),
        slippage: swapForm.slippage / 100, // Convert percentage to decimal
        userPrincipal: user?.principal || 'anonymous'
      };

      console.log('Executing swap with IC DEX:', swapRequest);
      
      const swapResult = await ICDEXService.executeSwap(swapRequest);
      
      console.log('IC DEX swap result:', swapResult);
      
      if (swapResult.success) {
        setSwapStatus('success');
        
        const swapRecord = {
          id: swapResult.transactionId || Date.now().toString(),
