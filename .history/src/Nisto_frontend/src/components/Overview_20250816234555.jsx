...
function Overview() {
  ...
  return (
    <div className="overview-container">
      ...
      {/* Sleek Assets Overview Card */}
      <div className="assets-overview-section">
        <div className="assets-overview-card">
          <div className="assets-overview-header">
            <div className="assets-overview-title">
              <FiGridIcon className="assets-overview-icon" />
              <span>Your Assets</span>
            </div>
            <div className="assets-overview-actions">
              <button className="assets-overview-action-btn">
                <FiPlus />
                Add Asset
              </button>
              <button className="assets-overview-action-btn">
          ...
          <div className="assets-overview-content">
            <div className="assets-overview-stats">
              <div className="asset-stat-item">
                <span className="asset-stat-label">Total Assets</span>
                <span className="asset-stat-value">{mockAssets.length}</span>
              </div>
              <div className="asset-stat-item">
                <span className="asset-stat-label">Largest Holding</span>
                <span className="asset-stat-value">Bitcoin (45%)</span>
              </div>
              <div className="asset-stat-item">
                <span className="asset-stat-label">Best Performer</span>
                <span className="asset-stat-value positive">Solana (+8.3%)</span>
              </div>
            </div>
            
            <div className="assets-overview-list">
              {mockAssets.map((asset, index) => (
                <div 
                  key={index} 
                  className="asset-overview-item"
                  onClick={() => handleAssetClick(asset)}
                >
                  <div className="asset-overview-info">
                    <div className="asset-overview-icon-wrapper" style={{ backgroundColor: asset.color }}>
                      {asset.icon}
                    </div>
                    <div className="asset-overview-details">
                      <div className="asset-overview-name">{asset.name}</div>
                      <div className="asset-overview-balance">
                        {asset.balance} {asset.symbol}
                      </div>
                    </div>
                  </div>
                  
                  <div className="asset-overview-values">
                    <div className="asset-overview-value">
                      {showBalances ? formatCurrency(asset.value) : '••••••'}
                    </div>
                    <div className="asset-overview-percentage">
                      {asset.percentage}%
                    </div>
                  </div>
                  
                  <div className="asset-overview-change">
                    <div className={`asset-change-indicator ${asset.priceChange >= 0 ? 'positive' : 'negative'}`}>
                      {asset.priceChange >= 0 ? '+' : ''}{asset.priceChange}%
                    </div>
                    <div className="asset-overview-price">
                      {asset.symbol === 'USDC' ? '$1.00' : `$${asset.price.toLocaleString()}`}
                    </div>
                  </div>
                  
                  <div className="asset-overview-actions">
                    <button className="asset-quick-btn">Send</button>
                    <button className="asset-quick-btn">Receive</button>
                  </div>
                </div>
              ))}
            </div>
