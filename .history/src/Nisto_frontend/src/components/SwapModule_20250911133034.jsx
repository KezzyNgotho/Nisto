...
const SwapModule = ({ onBack }) => {
  ...
  const handleSwap = async () => {
    ...
    try {
      // Execute swap using 1inch service
      const swapRequest = {
        fromToken: swapForm.fromCurrency,
        toToken: swapForm.toCurrency,
        amount: parseFloat(swapForm.fromAmount),
        slippage: swapForm.slippage, // Keep as percentage for 1inch
        userAddress: user?.principal || 'anonymous'
      };

      console.log('🔄 SwapModule: Executing swap with 1inch:', swapRequest);
      
      const swapResult = await OneInchService.executeSwap(swapRequest);
      
      console.log('✅ SwapModule: 1inch swap result:', swapResult);
