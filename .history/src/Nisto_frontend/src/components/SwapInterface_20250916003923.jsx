...
const SwapInterface = ({ onSwapComplete }) => {
  ...
  // Fetch real-time rate when fromToken or toToken changes
  useEffect(() => {
    if (fromToken && toToken) {
      fetchRealTimeRate(fromToken, toToken);
    }
  }, [fromToken, toToken, selectedProvider]);
  ...
  const fetchRealTimeRate = async (fromToken, toToken) => {
    ...
    try {
      ...
      // Try to get rate from selected provider
      if (selectedProvider === '1inch') {
        try {
          rate = await OneInchService.getRealTimePrice(fromTokenData.symbol, toTokenData.symbol);
          provider = '1inch';
        } catch (error) {
          console.warn('1inch rate not available:', error);
        }
      } else if (selectedProvider === 'icp') {
        try {
          rate = await ICDEXService.getExchangeRate(fromTokenData.symbol, toTokenData.symbol);
          provider = 'ICP DEX';
        } catch (error) {
          console.warn('ICP DEX rate not available:', error);
        }
      } else if (selectedProvider === 'auto') {
        // Try 1inch first, then ICP DEX
        try {
          rate = await OneInchService.getRealTimePrice(fromTokenData.symbol, toTokenData.symbol);
          provider = '1inch';
        } catch (error) {
          try {
            ...
            console.warn('Both providers failed:', error2);
          }
        }
      }

      // Fallback to calculated rate if no provider rate available
      if (rate <= 0) {
        rate = calculateExchangeRate(fromToken, toToken);
        provider = 'Calculated';
      }
                ...
                <div className="rate-info">
                  <div className="rate-main">
                    ...
                    <span className="rate-value">
                      1 {currentRate.fromToken} = {currentRate.rate.toFixed(6)} {currentRate.toToken}
                    </span>
                  </div>
                  <div className="rate-details">
                    <span className="rate-provider">via {currentRate.provider}</span>
                    <span className="rate-time">
                      {new Date(currentRate.timestamp).toLocaleTimeString()}
                    </span>
                  </div>
                </div>
                {rateLoading && (
