  ...
  return (
    <div className="deposit-container">
      <button className="btn btn-primary" onClick={() => setIsModalOpen(true)} disabled={loading}>
        <FiSmartphone /> Deposit
      </button>
      {isModalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => !loading && setIsModalOpen(false)}>
          <div style={{ background: '#0b0f1a', borderRadius: '12px', width: '360px', maxWidth: '90vw', padding: '16px', boxShadow: '0 10px 30px rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.06)' }} onClick={(e) => e.stopPropagation()}>
      {/* Header */}
