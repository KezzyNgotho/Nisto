...
const SocialMediaDemo = () => {
  ...
  useEffect(() => {
    ...
    const initializeIntegration = async () => {
      try {
        ...
        setSocialHandler(socialMediaIntegration);

        // Get initial stats
        updateStats(socialMediaIntegration);
        updateHealthStatus(socialMediaIntegration);
  ...
  const updateHealthStatus = async (handler) => {
    if (handler) {
      const health = await handler.getSystemHealth();
