...
export default function NotificationTestPanel() {
  ...
  const testCreateNotification = async () => {
    try {
      // Create a System notification type
      ...
      const result = await createNotification(
        ...
        'This notification was created via backend API',
        'medium',
        null, // actionUrl
        null  // expiresAt
      );
  ...
  const testCreateSecurityNotification = async () => {
    try {
      const notificationType = {
        Security: {
          event: 'Login Attempt',
      ...
      if (result) {
        showToast({
          ...
          message: 'Security notification created'
        });
        loadBackendNotifications();
        loadUnreadCount();
      }
  ...
  const testCreateTransactionNotification = async () => {
    try {
      const notificationType = {
        Transaction: {
          amount: 100.0,
      ...
      const result = await createNotification(
        notificationType,
        'Payment Received',
        'You received 100 USD from vault contribution',
        'medium',
        null,
        null
      );
      ...
      if (result) {
        showToast({
          ...
          message: 'Transaction notification created'
        });
        loadBackendNotifications();
        loadUnreadCount();
      }
                    ...
                    {notification.message}
                      ...
                      <span style={{
                        ...
                        background: notification.read ? 
                          ...
                          (theme === 'dark' ? '#7F1D1D' : '#FEE2E2'),
                        borderRadius: '4px',
                        color: notification.read ? colors.status.success : colors.status.error
                      }}>
