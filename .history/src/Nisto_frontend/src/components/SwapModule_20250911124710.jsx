...
const SwapModule = ({ onBack }) => {
  ...
  const calculateRate = async (fromCurrency, toCurrency) => {
    ...
    try {
      // Use 1inch for real-time rates from actual liquidity pools
      console.log(`🦄 SwapModule: Trying 1inch for real-time price...`);
      const rate = await OneInchService.getRealTimePrice(fromCurrency, toCurrency);
      
      if (rate > 0) {
        console.log(`✅ SwapModule: 1inch rate found: ${rate}`);
        return rate;
      }
      
      // Fallback to MarketDataService if 1inch doesn't have the pair
      console.log(`📡 SwapModule: 1inch no rate, trying MarketDataService...`);
      const fallbackRate = await MarketDataService.getExchangeRate(fromCurrency, toCurrency);
      console.log(`✅ SwapModule: MarketDataService rate: ${fallbackRate}`);
      return fallbackRate;
    } catch (error) {
