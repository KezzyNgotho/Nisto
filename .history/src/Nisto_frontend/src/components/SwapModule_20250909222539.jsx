...
const SwapModule = ({ onBack }) => {
  ...
  const calculateRate = async (fromCurrency, toCurrency) => {
    try {
      // Use backend service for real-time rates
      const rate = await swapBackendService.calculateExchangeRate(fromCurrency, toCurrency);
      return rate;
    } catch (error) {
  ...
  const performSearch = async (searchTerm) => {
    ...
    setIsSearching(true);
    try {
      // Search using backend service
      const searchData = await swapBackendService.searchCurrencies(searchTerm);
