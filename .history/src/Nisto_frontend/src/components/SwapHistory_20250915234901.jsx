...
const SwapHistory = ({ swapHistory = [], isLoading = false }) => {
  ...
  const [smartWalletHistory, setSmartWalletHistory] = useState([]);
  const [loadingSmartWallet, setLoadingSmartWallet] = useState(false);

  // Load SmartWallet history
  useEffect(() => {
    loadSmartWalletHistory();
  }, []);

  const loadSmartWalletHistory = async () => {
    try {
      setLoadingSmartWallet(true);
      const userId = 'user_' + Date.now(); // In production, get from auth context
      const history = await SmartWalletService.getSwapHistory(userId);
      setSmartWalletHistory(history);
    } catch (error) {
      console.error('Failed to load SmartWallet history:', error);
    } finally {
      setLoadingSmartWallet(false);
    }
  };

  // Combine local and SmartWallet history
  const combinedHistory = useMemo(() => {
    const allHistory = [...swapHistory, ...smartWalletHistory];
    return allHistory.sort((a, b) => (b.timestamp || b.id) - (a.timestamp || a.id));
  }, [swapHistory, smartWalletHistory]);
  ...
  const filteredSwaps = combinedHistory.filter(swap => {
  ...
  const autoSwaps = combinedHistory.filter(s => s.swapType === 'auto');
  const manualSwaps = combinedHistory.filter(s => s.swapType === 'manual');
      ...
      <div className="history-header">
        ...
        <div className="header-actions">
          <div className="history-stats">
            <span className="total-swaps">{combinedHistory.length} total swaps</span>
          </div>
          <button 
            className="refresh-button" 
            onClick={loadSmartWalletHistory}
            disabled={loadingSmartWallet}
          >
            <FiRefreshCw className={`refresh-icon ${loadingSmartWallet ? 'spinning' : ''}`} />
            <span>{loadingSmartWallet ? 'Loading...' : 'Refresh'}</span>
          </button>
        </div>
        </div>
