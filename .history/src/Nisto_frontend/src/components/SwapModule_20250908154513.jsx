...
const SwapModule = ({ onBack }) => {
  ...
  const loadCurrencies = async () => {
    ...
    try {
      ...
      // Combine all currencies and remove any duplicates
      const allCurrencies = [nstToken, ...transformedCurrencies, kesCurrency];
      const finalCurrencies = allCurrencies.filter((crypto, index, self) => 
        index === self.findIndex(c => c.symbol.toLowerCase() === crypto.symbol.toLowerCase())
      );
  ...
  const handleFromCurrencyChange = (currencyId) => {
    ...
    if (currency) {
      setSwapForm(prev => ({
        ...
        rate: Number(calculateRate(currencyId, prev.toCurrency)) || 0,
        fromAmount: '',
        toAmount: ''
      }));
  ...
  const handleToCurrencyChange = (currencyId) => {
    ...
    if (currency) {
      setSwapForm(prev => ({
        ...
        rate: Number(calculateRate(prev.fromCurrency, currencyId)) || 0,
        fromAmount: '',
        toAmount: ''
      }));
