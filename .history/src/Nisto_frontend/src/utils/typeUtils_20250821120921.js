...
function toMilliseconds(epochLike) {
  ...
  if (typeof epochLike === 'bigint') {
    // BigInt → Number (may overflow if too large; we handle thresholds before casting)
...
export function convertLargeNumbers(value) {
  ...
  if (typeof value === 'object') {
    ...
    if (typeof value.toText === 'function') {
      try { return value.toText(); } catch { /* ignore */ }
    }
    const out = {};
...
export function sanitizeForSerialization(obj) {
  ...
  const stampKeys = new Set([
    'createdAt','updatedAt','lastLoginAt','timestamp','created_at','updated_at','last_login_at'
  ]);
  ...
  function transform(value, key) {
    ...
    if (Array.isArray(value)) return value.map((v) => transform(v, undefined));
    if (typeof value === 'object') {
      const out = {};
      for (const [k, v] of Object.entries(value)) {
        out[k] = transform(v, k);
