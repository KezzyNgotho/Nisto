...
export const setupGlobalErrorHandling = () => {
  ...
  console.error = (...args) => {
    ...
    // Log to a service in production (if needed)
    if (process.env.NODE_ENV === 'production') {
      // You could send errors to a logging service here
      // console.log('Error logged to service:', args);
    }
  };
};
...
export const handleBackendError = (error) => {
  if (error.message?.includes('Canister has no update method')) {
    console.warn('Backend method not found, this might be a deployment issue');
    return { error: 'Service temporarily unavailable', retry: true };
  }
  
  if (error.message?.includes('BigInt')) {
    console.warn('BigInt serialization error');
    return { error: 'Data format error', retry: false };
  }
  
  return { error: 'An unexpected error occurred', retry: true };
};
