...
function Landing() {
  ...
  const handleInstallExtension = () => {
    ...
    try {
      // For development, we'll redirect to the extension files
      // In production, this would link to the Chrome Web Store
      const extensionUrl = process.env.NODE_ENV === 'production' 
        ? 'https://chrome.google.com/webstore/detail/nesto-ic-wallet/...' // Replace with actual store URL
        : '/manifest-v3.json'; // Local development
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        const instructions = `
🚀 Install Nesto Extension (Development Mode)

1. Download the extension files from: ${window.location.origin}/manifest-v3.json
2. Open Chrome and go to: chrome://extensions/
3. Enable "Developer mode" (toggle in top right)
4. Click "Load unpacked" and select the extension folder
5. The Nesto extension will appear in your toolbar!

For production installation, visit our Chrome Web Store listing.
        `;
        alert(instructions);
      }
    } catch (error) {
      console.error('Extension installation error:', error);
      alert('Unable to install extension. Please try again or contact support.');
    }
  };
