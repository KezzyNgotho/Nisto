...
export default function Profile() {
  ...
  const { user, principal, cryptoWallets, recoveryMethods, updateProfile, refreshRecoveryMethods, getUserRecoveryMethods } = useAuth();
  ...
  // Load recovery methods on component mount
  useEffect(() => {
    const loadRecoveryMethods = async () => {
      ...
      try {
        console.log('Profile: Loading recovery methods...');
        await refreshRecoveryMethods();
        console.log('Profile: Recovery methods loaded successfully');
        showToast({ message: 'Recovery methods loaded successfully', type: 'success' });
      } catch (error) {
        console.error('Profile: Error loading recovery methods:', error);
        handleError(error, 'recovery', showToast);
      } finally {
    ...
    loadRecoveryMethods();
  }, [user, refreshRecoveryMethods, showToast]);
  ...
  const onSave = async () => {
    ...
    console.log('onSave: username:', username, 'trimmed:', trimmed);
    ...
    try {
      ...
      handleSuccess('update', 'profile', showToast);
      setMessage('Profile updated successfully!');
    } catch (e) {
      const errorMessage = handleError(e, 'profile', showToast);
  ...
  const handleRefreshRecoveryMethods = async () => {
    setLoadingRecovery(true);
    try {
      await refreshRecoveryMethods();
      showToast({ message: 'Recovery methods refreshed successfully', type: 'success' });
    } catch (error) {
      handleError(error, 'recovery', showToast);
    } finally {
      setLoadingRecovery(false);
  ...
  const iconForRecovery = (type) => {
    switch (type) {
      ...
      case 'authenticator': return <FiSmartphone size={16} />;
      case 'security': return <FiKey size={16} />;
  ...
  const formatDate = (timestamp) => {
    ...
    try {
      ...
      if (typeof timestamp === 'number') {
        ...
        // Check if it's a BigInt timestamp (nanoseconds) from IC
        if (timestamp > 1000000000000000) {
          console.log('formatDate: detected BigInt timestamp, converting from nanoseconds to milliseconds');
  ...
  const formatDateTime = (timestamp) => {
    ...
    try {
      ...
      if (typeof timestamp === 'number') {
        ...
        // Check if it's a BigInt timestamp (nanoseconds) from IC
        if (timestamp > 1000000000000000) {
          console.log('formatDateTime: detected BigInt timestamp, converting from nanoseconds to milliseconds');
          date = new Date(timestamp / 1000000);
        } else {
          date = new Date(timestamp);
        }
  ...
  return (
      ...
      {message && (
        <div style={{
          background: 'linear-gradient(135deg, #10b981 0%, #05966 100%)',
          color: 'white',
          padding: '1rem',
          ...
          <div style={{
            ...
            <div style={{ position: 'relative', display: 'inline-block', marginBottom: '1.5rem' }}>
              <div style={{
                ...
                height: '100px',
                borderRadius: '50%',
                background: avatarUrl ? `url(${avatarUrl})` : `linear-gradient(135deg, ${theme.colors.primary} 0%, ${theme.colors.primary}dd 100%)`,
                ...
                fontSize: '2rem',
                fontWeight: 'bold',
                color: avatarUrl ? 'transparent' : theme.colors.white,
                margin: '0 auto',
                position: 'relative',
            ...
            <div style={{
              ...
              <div style={{ width: '6px', height: '6px', background: theme.colors.white, borderRadius: '50%' }}></div>
              Active Account
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '1.25rem', fontWeight: 700, color: theme.colors.info }}>{walletCount}</div>
                <div style={{ fontSize: '0.75rem', color: theme.colors.text.secondary }}>Wallets</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '1.25rem', fontWeight: 700, color: theme.colors.success }}>{recoveryCount}</div>
                <div style={{ fontSize: '0.75rem', color: theme.colors.text.secondary }}>Recovery</div>
              </div>
            </div>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <FiLock size={16} color={theme.colors.success} />
                  <span style={{ fontSize: '0.8rem', color: theme.colors.text.primary }}>Account Security</span>
                </div>
                <div style={{ color: theme.colors.success, fontSize: '0.8rem', fontWeight: 500 }}>Secure</div>
              </div>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <FiMail size={16} color={theme.colors.info} />
                  <span style={{ fontSize: '0.8rem', color: theme.colors.text.primary }}>Email Verified</span>
                </div>
                <div style={{ color: theme.colors.success, fontSize: '0.8rem', fontWeight: 500 }}>✓</div>
              </div>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <span style={{ fontSize: '0.8rem', color: theme.colors.text.primary }}>Recovery Methods</span>
                </div>
                <div style={{ color: recoveryCount > 0 ? theme.colors.success : theme.colors.error, fontSize: '0.8rem', fontWeight: 500 }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gap: '1rem' }}>
              <div style={{
                ...
                <div>
                  <div style={{ fontSize: '0.75rem', color: theme.colors.text.secondary, marginBottom: '0.25rem' }}>Principal ID</div>
                                      <div style={{ fontFamily: 'monospace', fontSize: '0.8rem', color: theme.colors.text.primary, fontWeight: 500 }}>
              ...
              <div style={{
                ...
                <div>
                  <div style={{ fontSize: '0.75rem', color: theme.colors.text.secondary, marginBottom: '0.25rem' }}>Joined</div>
                  <div style={{ fontSize: '0.8rem', color: theme.colors.text.primary, fontWeight: 500 }}>
                    {user?.createdAt ? formatDate(user.createdAt) : '—'}
                  </div>
                </div>
                                  <FiCalendar size={16} color={theme.colors.text.secondary} />
              </div>
              ...
              <div style={{
                ...
                <div>
                  <div style={{ fontSize: '0.75rem', color: theme.colors.text.secondary, marginBottom: '0.25rem' }}>Last Login</div>
                  <div style={{ fontSize: '0.8rem', color: theme.colors.text.primary, fontWeight: 500 }}>
                    {user?.lastLoginAt ? formatDateTime(user.lastLoginAt) : '—'}
                  </div>
                </div>
                                  <FiClock size={16} color={theme.colors.text.secondary} />
        </div>
          ...
          <div style={{
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <button 
                  onClick={handleRefreshRecoveryMethods}
                  ...
                  style={{
                    ...
                    borderRadius: '0.375rem',
                    color: theme.colors.text.secondary,
                    cursor: loadingRecovery ? 'not-allowed' : 'pointer',
                    fontSize: '0.8rem',
                    fontWeight: 500,
                    display: 'flex',
                    alignItems: 'center',
              ...
              {(recoveryMethods || []).map((method, idx) => (
                <div key={idx} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <div>
                      <div style={{ fontSize: '0.8rem', fontWeight: 600, color: theme.colors.text.primary }}>
                      ...
                      {method.createdAt && (
                        <div style={{ fontSize: '0.75rem', color: '#94a3b8', marginTop: '0.25rem' }}>
                          Added {new Date(parseInt(method.createdAt)).toLocaleDateString()}
                        </div>
                      )}
                    </div>
                  </div>
