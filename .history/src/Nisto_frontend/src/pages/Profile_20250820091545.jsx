    ...
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: 'grid', gap: 16, gridTemplateColumns: '1fr', maxWidth: 720 }}>
        ...
        <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 16, background: '#0b0f1a' }}>
          <div style={{ fontWeight: 700, marginBottom: 8 }}>Username</div>
          <input
            value={username}
            onChange={(e) => setUsername(e.target.value)}
            placeholder="Enter your username"
            style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.12)', background: 'transparent', color: '#fff' }}
          />
        ...
        <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 16, background: '#0b0f1a' }}>
          ...
          <InfoRow label="Principal">
            <span style={{ fontFamily: 'monospace', fontSize: 12, marginRight: 8 }}>{principal ? `${principal.slice(0,12)}...${principal.slice(-6)}` : '—'}</span>
            <button className="btn btn-text" onClick={() => navigator.clipboard.writeText(principal || '')}><FiCopy /> Copy</button>
          </InfoRow>
