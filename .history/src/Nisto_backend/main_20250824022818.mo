 ...
 // ========== PLUGIN SYSTEM HELPER FUNCTIONS ==========
        ...
        switch (vaultDetails) {
          ...
          case (?vault) {
            ...
            let notification: Notification = {
              ...
              message = "You've been invited to join vault: " # vault.name # " as " # role;
              priority = "high";
              isRead = false;
              isArchived = false;
              createdAt = now;
              readAt = null;
              metadata = ?("{\"vaultId\":\"" # vaultId # "\",\"role\":\"" # role # "\",\"invitedBy\":\"" # Principal.toText(caller) # "\"}");
            ...
            let currentUserNotifications = Option.get(userChatNotificationIndex.get(userId), []);
            userChatNotificationIndex.put(userId, Array.append(currentUserNotifications, [notificationId]));
            ...
            let currentInviterNotifications = Option.get(userChatNotificationIndex.get(caller), []);
  ...
  public shared(msg) func respondToVaultInvitation(
    ...
    switch (userMember) {
      ...
      case (?memberId) {
        switch (vaultMembers.get(memberId)) {
          case null { return #err("Member record not found"); };
          case (?member) {
            if (not member.isActive) {
              return #err("Member is not active");
            };
            
            let now = getCurrentTime();
            
            if (response == "accept") {
              // Update member status to active (if not already)
              let updatedMember: VaultMember = {
                id = member.id;
                vaultId = member.vaultId;
                userId = member.userId;
                role = member.role;
                joinedAt = member.joinedAt;
                contributionLimit = member.contributionLimit;
                withdrawalLimit = member.withdrawalLimit;
                isActive = true;
                permissions = member.permissions;
              };
              
              vaultMembers.put(memberId, updatedMember);
              ...
              switch (vaultDetails) {
                ...
                case (?vault) {
                  ...
                  let notification: Notification = {
                    ...
                    createdAt = now;
                    readAt = null;
                    metadata = ?("{\"vaultId\":\"" # vaultId # "\",\"memberId\":\"" # Principal.toText(caller) # "\",\"role\":\"" # member.role # "\"}");
                    actionUrl = ?("/vaults/" # vaultId);
                    expiresAt = ?(now + 24 * 60 * 60 * 1_000_000_000); // 24 hours
                  };
                  ...
                  let currentOwnerNotifications = Option.get(userChatNotificationIndex.get(vault.ownerId), []);
                  userChatNotificationIndex.put(vault.ownerId, Array.append(currentOwnerNotifications, [notificationId]));
                };
              };
