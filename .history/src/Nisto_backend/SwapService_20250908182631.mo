...
actor SwapService {
  ...
  private func makeHttpRequest(url: Text): async Result.Result<Text, Text> {
    ...
    try {
      ...
      if (response.status_code == 200) {
        ...
        #err("HTTP error: " # Nat.toText(Nat64.toNat(Nat64.fromNat16(response.status_code))));
      };
    } catch (error) {
      #err("Request failed");
    };
  ...
  // Helper function for icon and color mapping
  ...
  public func clearAllData(): async () {
    // Clear currencies
    for ((key, _) in currencies.entries()) {
      currencies.delete(key);
    };
    
    // Clear swap history
    for ((key, _) in swapHistory.entries()) {
      swapHistory.delete(key);
    };
    
    // Clear user swap history
    for ((key, _) in userSwapHistory.entries()) {
      userSwapHistory.delete(key);
    };
    
    // Clear market data
    for ((key, _) in marketData.entries()) {
      marketData.delete(key);
    };
    
    // Clear gas fees
    for ((key, _) in gasFees.entries()) {
      gasFees.delete(key);
    };
    
    // Clear price cache
    for ((key, _) in priceCache.entries()) {
