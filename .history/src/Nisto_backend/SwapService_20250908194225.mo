...
persistent actor SwapService {
  ...
  public func calculateExchangeRate(fromCurrency: Text, toCurrency: Text): async Float {
    // Simple rate calculation - in production you'd use real exchange rates
    switch (fromCurrency, toCurrency) {
      case ("BTC", "USDT") { 112000.0 };
      case ("ETH", "USDT") { 3500.0 };
      case ("USDT", "BTC") { 1.0 / 112000.0 };
      case ("USDT", "ETH") { 1.0 / 3500.0 };
      case ("BTC", "ETH") { 112000.0 / 3500.0 };
      case ("ETH", "BTC") { 3500.0 / 112000.0 };
      case (_, _) { 1.0 }; // Default rate
    };
  };
