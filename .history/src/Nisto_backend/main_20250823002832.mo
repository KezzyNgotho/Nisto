...
actor Nisto {
  ...
  public type NotificationType = variant {
  ...
  public type CryptoWalletType = variant {
    #ICP;
    #ckBTC;
    #ckETH;
    #CHAT;
  ...
  // Admin principal
  private let admin: Principal = Principal.fromText("2vxsx-fae"); // Anonymous principal for development
  ...
  private var notifications = HashMap.HashMap<Text, Notification>(500, Text.equal, Text.hash);
  private var notificationPreferences = HashMap.HashMap<UserId, NotificationPreferences>(10, Principal.equal, Principal.hash);
  private var pushSubscriptions = HashMap.HashMap<Text, PushSubscription>(100, Text.equal, Text.hash);
  private stable var nextNotificationId: Nat = 0;
  private stable var nextPushSubscriptionId: Nat = 0;
  
  // Notification indexes
  private var userNotifications = HashMap.HashMap<UserId, [Text]>(10, Principal.equal, Principal.hash); // userId -> notificationIds
  private var userPushSubscriptions = HashMap.HashMap<UserId, [Text]>(10, Principal.equal, Principal.hash); // userId -> subscriptionIds
 ...
 // ========== PLUGIN SYSTEM HELPER FUNCTIONS ==========
  ...
  private var simplePaymentsMap = HashMap.HashMap<Text, SimplePayment>(0, Text.equal, Text.hash);
  private stable var nextSimplePaymentId: Nat = 1;
  ...
  // Paybill storage
  private stable var nextPaybillTransactionId: Nat = 1;
  private var paybillTransactionsMap = HashMap.HashMap<Text, PaybillTransaction>(0, Text.equal, Text.hash);
  
  // Deposit storage
  private stable var nextDepositTransactionId: Nat = 1;
  private var depositTransactionsMap = HashMap.HashMap<Text, DepositTransaction>(0, Text.equal, Text.hash);
  
  // Available bills (in production, this would be fetched from a database)
  private let availableBills: [PaybillBill] = [
    {
      id = "mpesa-paybill";
      name = "Mpesa Paybill";
  ...
  public shared(msg) func createNotification(
    ...
    switch (users.get(caller)) {
      ...
      case (?user) {
        ...
        await sendPushNotification(caller, notification);
  ...
  public shared query(msg) func getUserNotifications(
    limit: Nat,
    offset: Nat
  ): async Result.Result<[Notification], Text> {
    let caller = msg.caller;
    ...
    switch (userNotifications.get(caller)) {
      ...
      case (?notificationIds) {
        ...
        // Sort by creation date (newest first)
        let sortedNotifications = Array.sort<Notification>(
          allNotifications,
        ...
        if (startIndex >= sortedNotifications.size()) {
          ...
          let paginatedNotifications = Array.subArray<Notification>(sortedNotifications, startIndex, endIndex - startIndex);
    ...
    switch (notifications.get(notificationId)) {
      ...
      case (?notification) {
        if (notification.userId != caller) {
          #err("Not authorized to read this notification")
        } else {
  ...
  public shared(msg) func markAllNotificationsAsRead(): async Result.Result<Bool, Text> {
    let caller = msg.caller;
      ...
      case (?notificationIds) {
        for (notificationId in notificationIds.vals()) {
          switch (notifications.get(notificationId)) {
            case null { };
            case (?notification) {
              if (notification.userId == caller and not notification.isRead) {
                let updatedNotification: Notification = {
                  id = notification.id;
                  userId = notification.userId;
                  type = notification.type;
  ...
  public shared(msg) func deleteNotification(notificationId: Text): async Result.Result<Bool, Text> {
    ...
    switch (notifications.get(notificationId)) {
      ...
      case (?notification) {
        if (notification.userId != caller) {
          #err("Not authorized to delete this notification")
        } else {
          // Remove from storage
          ...
          switch (userNotifications.get(caller)) {
            case null { };
            case (?notificationIds) {
              let filteredIds = Array.filter<Text>(notificationIds, func(id) { id != notificationId });
              userNotifications.put(caller, filteredIds);
            };
          };
  ...
  public shared(msg) func addPushSubscription(
    ...
    let subscriptionId = generateId("push", nextPushSubscriptionId);
    ...
    // Store subscription
    pushSubscriptions.put(subscriptionId, subscription);
    ...
    switch (pushSubscriptions.get(subscriptionId)) {
      ...
      case (?subscription) {
        if (subscription.userId != caller) {
          #err("Not authorized to remove this subscription")
        } else {
          // Remove from storage
          ...
          switch (userPushSubscriptions.get(caller)) {
            ...
            case (?subscriptionIds) {
              let filteredIds = Array.filter<Text>(subscriptionIds, func(id) { id != subscriptionId });
              userPushSubscriptions.put(caller, filteredIds);
            };
          };
  ...
  public shared(msg) func updateNotificationPreferences(
    ...
    let notificationPrefs: NotificationPreferences = {
      ...
      vaultNotifications = preferences.vaultNotifications;
      ...
      recoveryNotifications = preferences.recoveryNotifications;
      quietHours = preferences.quietHours;
      timezone = preferences.timezone;
      updatedAt = Time.now();
    };
    
    notificationPreferences.put(caller, notificationPrefs);
    #ok(notificationPrefs)
  };

  // Get notification preferences
  public shared query(msg) func getNotificationPreferences(): async Result.Result<NotificationPreferences, Text> {
    let caller = msg.caller;
  ...
  public shared(msg) func sendCrossCanisterNotification(
    ...
    // This would integrate with cross-canister calls
  ...
  private func sendPushNotification(userId: UserId, notification: Notification) {
    // This would integrate with a push service
    // For now, we'll just log it
    Debug.print("Push notification would be sent to user: " # Principal.toText(userId) # " for notification: " # notification.id);
  };

  // Get unread notification count
  public shared query(msg) func getUnreadNotificationCount(): async Result.Result<Nat, Text> {
    let caller = msg.caller;
