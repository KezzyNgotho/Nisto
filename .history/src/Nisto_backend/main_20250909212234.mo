...
persistent actor Nisto {
 ...
 // ========== PLUGIN SYSTEM HELPER FUNCTIONS ==========
  ...
  public func executeSwap(request: SwapRequest): async SwapResult {
    ...
    let expectedOutput = request.inputAmount * exchangeRate;
    let minOutput = expectedOutput * (1.0 - request.slippageTolerance);
    ...
    let swapResult: SwapResult = {
      ...
      exchangeRate = exchangeRate;
      slippage = (expectedOutput - minOutput) / expectedOutput;
      status = "completed";
      timestamp = getCurrentTime();
    ...
    swapHistory.put(swapId, swapResult);
    
    // Add to user's swap history
    switch (userSwapHistory.get(request.user)) {
      case null {
        userSwapHistory.put(request.user, [swapId]);
      };
      case (?existingSwaps) {
        let updatedSwaps = Array.append(existingSwaps, [swapId]);
        userSwapHistory.put(request.user, updatedSwaps);
      };
    };
