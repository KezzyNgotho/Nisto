...
persistent actor Nisto {
 ...
 // ========== PLUGIN SYSTEM HELPER FUNCTIONS ==========
  ...
  private transient var swapHistory = HashMap.HashMap<Text, SwapResult>(100, Text.equal, Text.hash);
  ...
  public func getAllCurrencies(): async [Currency] {
    // Market data is now fetched by the frontend
    // This function returns a basic list for backend operations
    [
      {
        id = "internet-computer";
        symbol = "ICP";
  ...
  public func executeSwap(request: SwapRequest): async SwapResult {
    ...
    let historyEntry = {
      ...
      status = "completed";
      timestamp = getCurrentTime();
      transactionHash = "0x" # Text.substring(swapId, 0, 10);
      gasFee = 0.001;
    };
