...
persistent actor Nisto {
 ...
 // ========== PLUGIN SYSTEM HELPER FUNCTIONS ==========
  ...
  public func getAllCurrencies(): async [Currency] {
    // REAL-TIME API DATA ONLY - NO HARDCODED VALUES
    // Using a simpler API endpoint that's more likely to work
    
    let simpleUrl = "https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,solana,tether,binancecoin,cardano,polkadot&vs_currencies=usd&include_24hr_change=true";
    
    switch (await makeHttpRequest(simpleUrl)) {
      case (#ok(responseText)) {
        Debug.print("API Response received: " # responseText);
        switch (parseSimplePriceResponse(responseText)) {
          case (#ok(currencies)) {
            return currencies;
          };
          case (#err(error)) {
      ...
      case (#err(error)) {
        Debug.print("HTTP request failed: " # error);
        return [];
      };
    };
  };
