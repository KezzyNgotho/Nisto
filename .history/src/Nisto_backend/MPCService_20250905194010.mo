...
actor MPCService {
  ...
  public query func getMPCTransactions(walletId: MPCWalletId): async [MPCTransaction] {
    ...
    Array.filter<MPCTransaction>(Array.fromIter(allTransactions), func(tx) = tx.walletId == walletId);
  };
  ...
  public query func getSocialTransfers(userId: UserId): async [SocialTransfer] {
    ...
    Array.filter<SocialTransfer>(Array.fromIter(allTransfers), func(transfer) = 
