...
persistent actor Nisto {
 ...
 // ========== PLUGIN SYSTEM HELPER FUNCTIONS ==========
  ...
  public func getAllCurrencies(): async [Currency] {
    ...
    let coinGeckoUrl = "https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=50&page=1&sparkline=false&price_change_percentage=24h";
    let coinCapUrl = "https://api.coincap.io/v2/assets?limit=50";
    
    // Try CoinGecko first
    switch (await makeHttpRequest(coinGeckoUrl)) {
      case (#ok(responseText)) {
        switch (parseCoinGeckoResponse(responseText)) {
          case (#ok(currencies)) {
            return currencies;
          };
          case (#err(_)) {
            // Try CoinCap as backup
            switch (await makeHttpRequest(coinCapUrl)) {
              case (#ok(responseText2)) {
                switch (parseCoinCapResponse(responseText2)) {
                  case (#ok(currencies2)) {
                    return currencies2;
                  };
                  case (#err(_)) {
