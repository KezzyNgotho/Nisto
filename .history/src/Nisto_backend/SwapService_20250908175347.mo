...
actor SwapService {
  ...
  private func fetchCryptocurrencies(): async Result.Result<[Currency], Text> {
    ...
    switch (await makeHttpRequest(url)) {
      case (#ok(responseText)) {
        // For now, use mock data but log that we got a response
  ...
  private func fetchCurrencyPrice(currencyId: Text): async Result.Result<Float, Text> {
    ...
    switch (await makeHttpRequest(url)) {
      case (#ok(responseText)) {
        // For now, use mock price but log that we got a response
        Debug.print("Got price response for " # currencyId # ", length: " # Nat.toText(Text.size(responseText)));
        // TODO: Implement proper JSON parsing
  ...
  // JSON parsing helper for currencies
  private func parseCurrencyFromJson(json: JSON.JSON): Result.Result<Currency, Text> {
    switch (JSON.getObject(json)) {
      case (#ok(obj)) {
        let id = switch (JSON.getObjectField(obj, "id")) {
          case (#ok(idJson)) {
            switch (JSON.getString(idJson)) {
              case (#ok(idText)) { idText };
              case (#err(_)) { "unknown" };
            };
          };
          case (#err(_)) { "unknown" };
        ...
        let symbol = switch (JSON.getObjectField(obj, "symbol")) {
          case (#ok(symbolJson)) {
            switch (JSON.getString(symbolJson)) {
              case (#ok(symbolText)) { Text.toUppercase(symbolText) };
              case (#err(_)) { "UNKNOWN" };
            };
          };
          case (#err(_)) { "UNKNOWN" };
        ...
        let name = switch (JSON.getObjectField(obj, "name")) {
          case (#ok(nameJson)) {
            switch (JSON.getString(nameJson)) {
              case (#ok(nameText)) { nameText };
              case (#err(_)) { "Unknown" };
            };
          };
          case (#err(_)) { "Unknown" };
        ...
        let price = switch (JSON.getObjectField(obj, "current_price")) {
          case (#ok(priceJson)) {
            switch (JSON.getNumber(priceJson)) {
              case (#ok(priceText)) { Float.fromInt(Int.abs(Int.fromText(priceText))) };
              case (#err(_)) { 0.0 };
            };
          };
          case (#err(_)) { 0.0 };
        ...
        let change24h = switch (JSON.getObjectField(obj, "price_change_percentage_24h")) {
          case (#ok(changeJson)) {
            switch (JSON.getNumber(changeJson)) {
              case (#ok(changeText)) { Float.fromInt(Int.abs(Int.fromText(changeText))) };
              case (#err(_)) { 0.0 };
            };
          };
          case (#err(_)) { 0.0 };
        ...
        let marketCap = switch (JSON.getObjectField(obj, "market_cap")) {
          case (#ok(mcJson)) {
            switch (JSON.getNumber(mcJson)) {
              case (#ok(mcText)) { ?Float.fromInt(Int.abs(Int.fromText(mcText))) };
              case (#err(_)) { null };
            };
          };
          case (#err(_)) { null };
        ...
        let volume = switch (JSON.getObjectField(obj, "total_volume")) {
          case (#ok(volJson)) {
            switch (JSON.getNumber(volJson)) {
              case (#ok(volText)) { ?Float.fromInt(Int.abs(Int.fromText(volText))) };
              case (#err(_)) { null };
            };
          };
          case (#err(_)) { null };
        ...
        let image = switch (JSON.getObjectField(obj, "image")) {
          case (#ok(imgJson)) {
            switch (JSON.getString(imgJson)) {
              case (#ok(imgText)) { imgText };
              case (#err(_)) { "https://via.placeholder.com/32" };
            };
          };
          case (#err(_)) { "https://via.placeholder.com/32" };
        };
        ...
        let (icon, color) = getIconAndColor(symbol);
  ...
  private func getIconAndColor(symbol: Text): (Text, Text) {
    let lowerSymbol = Text.toLowercase(symbol);
    switch (lowerSymbol) {
      case ("btc") { ("FiStar", "#F7931A") };
      case ("eth") { ("FiActivity", "#627EEA") };
      case ("usdt") { ("FiDollarSign", "#26A17B") };
      case ("usdc") { ("FiDollarSign", "#2775CA") };
      case ("icp") { ("FiGlobe", "#29BEEE") };
      case ("sol") { ("FiGlobe", "#9945FF") };
      case ("doge") { ("FiZap", "#C2A633") };
      case ("pepe") { ("FiZap", "#00D4AA") };
      case ("bnb") { ("FiTarget", "#F3BA2F") };
      case ("ada") { ("FiGlobe", "#0033AD") };
      case ("avax") { ("FiGlobe", "#E84142") };
      case ("link") { ("FiLink", "#2A5ADA") };
      case ("dot") { ("FiGlobe", "#E6007A") };
      case ("ltc") { ("FiStar", "#BFBBBB") };
      case ("uni") { ("FiTarget", "#FF007A") };
      case (_) { ("FiCircle", "#666666") };
    };
  };
