...
actor SwapService {
    ...
    try {
      ...
      if (response.status == 200) {
        switch (response.body) {
          ...
          case (null) { #err("Empty response body") };
        };
      } else {
  ...
  private func fetchCryptocurrencies(): async Result.Result<[Currency], Text> {
    let url = COINGECKO_BASE_URL # "/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=250&page=1&sparkline=false&price_change_percentage=24h";
    
    switch (await makeHttpRequest(url)) {
      case (#ok(responseText)) {
        // Parse JSON response and convert to Currency array
        // For now, we'll use mock data but this is where real parsing would go
  ...
  private func fetchCurrencyPrice(currencyId: Text): async Result.Result<Float, Text> {
    ...
    let url = COINGECKO_BASE_URL # "/simple/price?ids=" # currencyId # "&vs_currencies=usd";
  ...
  public func searchCurrencies(query: Text): async [Currency] {
