...
actor Nisto {
 ...
 // ========== PLUGIN SYSTEM HELPER FUNCTIONS ==========
  ...
  // Available payment services with colors and icons
  private let paymentServices: [PaymentService] = [
    {
      id = "pay-bills";
      name = "Pay Bills";
      icon = "FiDollarSign";
  ...
  public shared(msg) func getPaymentServices(): async Result.Result<[PaymentService], Text> {
    let caller = msg.caller;
    
    await logAuditEvent(?caller, "PAYMENT_SERVICES_FETCHED", "Fetched payment services", true);
