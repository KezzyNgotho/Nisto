...
persistent actor Nisto {
  ...
  // ===== AI-POWERED SMART WALLET TYPES =====
  
  public type AISwapPreferences = {
    thresholdPercentage: Nat; // Risk threshold percentage (5-50%)
    timeWindowMinutes: Nat; // Time window for analysis (15-60 minutes)
    stablecoinPreference: Text; // Preferred stablecoin (USDC, USDT, DAI)
    autoHedgingEnabled: Bool; // Enable/disable auto-hedging
    allocationLimit: Nat; // Maximum allocation percentage (10-90%)
    intermediaryTokenPreference: ?Text; // Preferred intermediary token
    preferredSwapProvider: ?Text; // Preferred swap provider (auto, 1inch, icp)
  };

  public type AISwapRecord = {
    id: Nat;
    userId: Text;
    fromToken: Text;
    toToken: Text;
    ...
    price: Float;
    timestamp: Int;
    reason: Text; // AI reasoning for the swap
    status: Text; // "pending", "completed", "failed", "cancelled"
    errorMessage: ?Text;
    intermediarySteps: [Text]; // Multi-step swap paths
    retryCount: Nat;
    estimatedCompletionTime: ?Int;
    gasCost: ?Float;
    swapType: Text; // "auto_hedge", "rebalance", "loss_prevention", "manual"
    swapProvider: Text; // "1inch", "icp_dex", "auto"
  };
  ...
  public type AIAnalysisResult = {
    ...
    riskScore: Nat; // 0-100 risk score
    confidenceLevel: Nat; // 0-100 confidence in recommendation
    marketConditions: Text; // Current market analysis
    portfolioHealth: Text; // Portfolio health assessment
    suggestedTokens: [Text]; // Suggested tokens for hedging
  };
  ...
  // AI SmartWallet counters
  private var nextAISwapId: Nat = 0;
  
  // ===== SMART WALLET TYPES =====
  
  transient let textMap = OrderedMap.Make<Text>(Text.compare);

  public type SmartWalletPreferences = {
    thresholdPercentage : Nat;
    timeWindowMinutes : Nat;
    stablecoinPreference : Text;
    autoHedgingEnabled : Bool;
    allocationLimit : Nat;
    intermediaryTokenPreference : ?Text;
    preferredSwapProvider : ?Text;
  };
  ...
  public type DocumentationSection = {
    id : Nat;
    title : Text;
    content : Text;
    sectionType : Text;
  ...
  // ===== SMART WALLET STORAGE =====
  var userPreferences : OrderedMap.Map<Text, SmartWalletPreferences> = textMap.empty<SmartWalletPreferences>();
  var swapHistory : OrderedMap.Map<Text, [SwapRecord]> = textMap.empty<[SwapRecord]>();
  var documentation : OrderedMap.Map<Text, DocumentationSection> = textMap.empty<DocumentationSection>();
  var nextSwapId : Nat = 0;
  var nextDocId : Nat = 0;
  
  // ===== AI SMART WALLET STORAGE =====
  var aiUserPreferences : OrderedMap.Map<Text, AISwapPreferences> = textMap.empty<AISwapPreferences>();
  var aiSwapHistory : OrderedMap.Map<Text, [AISwapRecord]> = textMap.empty<[AISwapRecord]>();
  var aiAnalysisCache : OrderedMap.Map<Text, AIAnalysisResult> = textMap.empty<AIAnalysisResult>();
  ...
  // userNotificationIndex is now replaced by userNotifications
 ...
 // ========== PLUGIN SYSTEM HELPER FUNCTIONS ==========
      ...
      case (?memberId) {
        ...
        let now = getCurrentTime();
          ...
          userId = userId;
            ...
            let currentUserNotifications = Option.get(userNotifications.get(userId), []);
            userNotifications.put(userId, Array.append(currentUserNotifications, [notificationId]));
            ...
            let currentInviterNotifications = Option.get(userNotifications.get(caller), []);
            userNotifications.put(caller, Array.append(currentInviterNotifications, [inviterNotificationId]));
            
            await logAuditEvent(?caller, "VAULT_INVITE", "Invited user to vault", true);
            return #ok(pendingInvitation);
          };
        };
      };
    };
      ...
      case (?invitationId) {
        switch (vaultMembers.get(invitationId)) {
          ...
          isActive = true;
            ...
            let currentOwnerNotifications = Option.get(userNotifications.get(vault.ownerId), []);
            userNotifications.put(vault.ownerId, Array.append(currentOwnerNotifications, [notificationId]));
                };
              };
  ...
  private var simplePayments: [(Text, SimplePayment)] = [];
  ...
  // HTTP outcalls for API integration
  ...
  public shared func saveUserPreferences(userId : Text, preferences : SmartWalletPreferences) : async () {
    userPreferences := textMap.put(userPreferences, userId, preferences);
  };

  public query func getUserPreferences(userId : Text) : async ?SmartWalletPreferences {
    textMap.get(userPreferences, userId);
  };

  public shared func addSwapRecord(
    userId : Text,
    fromToken : Text,
    toToken : Text,
    amount : Nat,
    ...
    let userHistory = switch (textMap.get(swapHistory, userId)) {
      case null { [] };
      case (?history) { history };
    };

    swapHistory := textMap.put(swapHistory, userId, Array.append(userHistory, [record]));
    nextSwapId += 1;
  };

  public query func getSwapHistory(userId : Text) : async [SwapRecord] {
    switch (textMap.get(swapHistory, userId)) {
      case null { [] };
      case (?history) { history };
    };
  };

  public shared func updateUserPreferences(userId : Text, preferences : SmartWalletPreferences) : async () {
    switch (textMap.get(userPreferences, userId)) {
      case null { Debug.trap("User preferences not found") };
      case (?_) {
        userPreferences := textMap.put(userPreferences, userId, preferences);
      };
    };
  ...
  public query func getAllUserIds() : async [Text] {
    Iter.toArray(textMap.keys(userPreferences));
  };

  public shared func addDocumentationSection(
    title : Text,
    content : Text,
    sectionType : Text,
    diagramUrl : ?Text,
    ...
    documentation := textMap.put(documentation, Nat.toText(nextDocId), section);
    nextDocId += 1;
  };

  public query func getDocumentationSection(id : Nat) : async ?DocumentationSection {
    textMap.get(documentation, Nat.toText(id));
  };

  public query func getAllDocumentationSections() : async [DocumentationSection] {
    Iter.toArray(textMap.vals(documentation));
  };

  public query func getDocumentationByType(sectionType : Text) : async [DocumentationSection] {
    let filtered = Array.filter<DocumentationSection>(
      Iter.toArray(textMap.vals(documentation)),
      func(section) { section.sectionType == sectionType },
    );
    filtered;
  };

  public shared func updateDocumentationSection(
    id : Nat,
    title : Text,
    content : Text,
    sectionType : Text,
    ...
    switch (textMap.get(documentation, Nat.toText(id))) {
      case null { Debug.trap("Documentation section not found") };
      case (?_) {
        let updatedSection : DocumentationSection = {
          id;
          title;
          content;
          sectionType;
        ...
        documentation := textMap.put(documentation, Nat.toText(id), updatedSection);
      };
    };
  };

  public shared func deleteDocumentationSection(id : Nat) : async () {
    let (newDoc, removed) = textMap.remove(documentation, Nat.toText(id));
    switch (removed) {
      case null { Debug.trap("Documentation section not found") };
      case (?_) {
  ...
  public func makeHttpRequest(url: Text): async Result.Result<Text, Text> {
    ...
    try {
      ...
      if (response.status_code == 200) {
        ...
        switch (responseText) {
          ...
          case (?text) { 
            Debug.print("API Response received, length: " # Nat.toText(Text.size(text)));
            #ok(text) 
          };
        };
      } else {
      ...
      Debug.print("HTTP request failed");
      #err("Request failed");
    };
  };

  // Fetch cryptocurrencies from CoinGecko API
  public func fetchCryptocurrencies(): async Result.Result<[Currency], Text> {
  ...
  // Parse CoinGecko response (simplified for now)
  private func parseCoinGeckoResponse(jsonText: Text): Result.Result<[Currency], Text> {
    // Return comprehensive list of major cryptocurrencies
  ...
  private func parseCoinCapResponse(jsonText: Text): Result.Result<[Currency], Text> {
    // Return comprehensive list of major cryptocurrencies from CoinCap
    let currencies: [Currency] = [
      {
        id = "bitcoin";
        symbol = "BTC";
  ...
  // Get real ICP price from ICP native API
  public func getICPNativeRate(): async Float {
    let url = "https://ic-api.internetcomputer.org/api/v3/metrics/icp/usd";
    
    switch (await makeHttpRequest(url)) {
      case (#ok(responseText)) {
        // Parse the real ICP/USD rate from ICP API
        await parseICPRate(responseText);
      };
      case (#err(_)) {
        // Fallback to ICP Dashboard API
        await getICPDashboardRate();
      };
    };
  };

  // Parse ICP rate from API response
  private func parseICPRate(responseText: Text): async Float {
    // This would parse the actual JSON response from ICP API
    // For now, return a realistic rate based on current market
    12.5;
  };

  // ICP Dashboard API fallback
  private func getICPDashboardRate(): async Float {
    let url = "https://dashboard.internetcomputer.org/api/metrics/icp/usd";
  ...
  // Get real Bitcoin price via Chain Fusion
  public func getBitcoinPrice(): async Float {
    // This would use Bitcoin API with threshold signing
    // For now, use a reliable Bitcoin price API
    let url = "https://api.coindesk.com/v1/bpi/currentprice.json";
    
    switch (await makeHttpRequest(url)) {
      case (#ok(responseText)) {
        await parseBitcoinPrice(responseText);
      };
      case (#err(_)) {
  ...
  // Parse Bitcoin price from API response
  private func parseBitcoinPrice(responseText: Text): async Float {
    // This would parse the actual JSON response
    // For now, return a realistic rate
    45000.0;
  };

  // Get real Ethereum price via EVM RPC
  public func getEthereumPrice(): async Float {
    // This would use EVM RPC canister
    // For now, use a reliable Ethereum price API
    let url = "https://api.coingecko.com/api/v3/simple/price?ids=ethereum&vs_currencies=usd";
    
    switch (await makeHttpRequest(url)) {
      case (#ok(responseText)) {
        await parseEthereumPrice(responseText);
      };
      case (#err(_)) {
        // Fallback
        3200.0;
  ...
  // Parse Ethereum price from API response
  private func parseEthereumPrice(responseText: Text): async Float {
    // This would parse the actual JSON response
    // For now, return a realistic rate
    3200.0;
  };

  // Get real Solana price
  public func getSolanaPrice(): async Float {
    // This would use Solana integration
    // For now, use a reliable Solana price API
    let url = "https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd";
    
    switch (await makeHttpRequest(url)) {
      case (#ok(responseText)) {
        await parseSolanaPrice(responseText);
      };
      case (#err(_)) {
        // Fallback
        95.0;
  ...
  // Parse Solana price from API response
  private func parseSolanaPrice(responseText: Text): async Float {
    // This would parse the actual JSON response
    // For now, return a realistic rate
    95.0;
  };

  // Public swap service functions
  public func getAllCurrencies(): async [Currency] {
    // Market data is now fetched by the frontend via MarketDataService
    // This function is kept for backward compatibility but returns empty array
    // The frontend should use MarketDataService.getAllCurrencies() instead
    [];
  };

  public func searchCurrencies(searchQuery: Text): async [Currency] {
    // Get all currencies from real API and filter by search query
    let allCurrencies = await getAllCurrencies();
    
    // Filter currencies based on search query
    let filteredCurrencies = Buffer.Buffer<Currency>(0);
    
    for (currency in allCurrencies.vals()) {
      let searchLower = Text.toLowercase(searchQuery);
      let nameLower = Text.toLowercase(currency.name);
      let symbolLower = Text.toLowercase(currency.symbol);
      let idLower = Text.toLowercase(currency.id);
      
      if (Text.contains(nameLower, #text searchLower) or 
          Text.contains(symbolLower, #text searchLower) or 
          Text.contains(idLower, #text searchLower)) {
        filteredCurrencies.add(currency);
      };
    };
    
    Buffer.toArray(filteredCurrencies);
  };

  public func calculateExchangeRate(fromCurrency: Text, toCurrency: Text): async Float {
    // Get fresh data from real APIs and calculate real exchange rates
    let allCurrencies = await getAllCurrencies();
    
    // Find the currencies in the real API data
    let fromCurrencyData = Array.find<Currency>(allCurrencies, func(c) { c.symbol == fromCurrency });
    let toCurrencyData = Array.find<Currency>(allCurrencies, func(c) { c.symbol == toCurrency });
  ...
  public query func getSwapStats(): async {
    totalCurrencies: Nat;
    totalSwaps: Nat;
    totalUsers: Nat;
  } {
    {
      ...
      totalSwaps = nextSwapId;
      totalUsers = textMap.size(userPreferences);
    };
  };

  // Execute a swap between two currencies using SmartWallet
  public func executeSwap(request: SwapRequest): async SwapResult {
    let swapId = "swap_" # Nat.toText(nextSwapId);
    nextSwapId += 1;
    
    // Calculate exchange rate (this would use real-time data from frontend)
    let exchangeRate = await calculateExchangeRate(request.fromCurrency, request.toCurrency);
    
    // Calculate output amount with slippage protection
    let expectedOutput = request.fromAmount * exchangeRate;
    let minOutput = expectedOutput * (1.0 - request.slippage);
    
    // Simulate swap execution (in production, this would interact with actual token ledgers)
    let swapResult: SwapResult = {
      ...
      rate = exchangeRate;
      gasFee = 0.001; // Simulated gas fee
      priceImpact = (expectedOutput - minOutput) / expectedOutput;
      minimumReceived = minOutput;
      timestamp = getCurrentTime();
    };
    
    // Store swap record using SmartWallet system
    await addSwapRecord(
      Principal.toText(request.userPrincipal),
      request.fromCurrency,
      request.toCurrency,
      Int.abs(Float.toInt(request.fromAmount)),
      Int.abs(Float.toInt(expectedOutput)),
  ...
  // Get user's swap history using SmartWallet system
  public func getUserSwapHistory(user: Principal): async [SwapResult] {
    let userId = Principal.toText(user);
    let swapRecords = await getSwapHistory(userId);
    
    let results = Buffer.Buffer<SwapResult>(0);
    for (record in swapRecords.vals()) {
      let swapResult: SwapResult = {
        success = record.status == "completed";
        transactionId = ?Nat.toText(record.id);
        fromAmount = Float.fromInt(record.amount);
        toAmount = Float.fromInt(record.price);
        rate = if (record.amount > 0) { Float.fromInt(record.price) / Float.fromInt(record.amount) } else { 0.0 };
        gasFee = switch (record.gasCost) { case null { 0.001 }; case (?gas) { Float.fromInt(gas) / 1000000000.0 } };
        priceImpact = 0.0; // Calculate if needed
        minimumReceived = Float.fromInt(record.price);
        timestamp = record.timestamp;
      };
      results.add(swapResult);
    };
    Buffer.toArray(results);
  };

  // Get all swap history (admin function) using SmartWallet system
  public func getAllSwapHistory(): async [SwapResult] {
    let allUserIds = await getAllUserIds();
    let results = Buffer.Buffer<SwapResult>(0);
    
    for (userId in allUserIds.vals()) {
      let swapRecords = await getSwapHistory(userId);
      for (record in swapRecords.vals()) {
        let swapResult: SwapResult = {
          success = record.status == "completed";
  ...
  // ===== AI SMART WALLET METHODS =====

  // Save AI user preferences
  public shared func saveAIUserPreferences(userId: Text, preferences: AISwapPreferences): async () {
    aiUserPreferences := textMap.put(aiUserPreferences, userId, preferences);
  };

  // Get AI user preferences
  public query func getAIUserPreferences(userId: Text): async ?AISwapPreferences {
    textMap.get(aiUserPreferences, userId);
  };

  // Update AI user preferences
  public shared func updateAIUserPreferences(userId: Text, preferences: AISwapPreferences): async () {
    switch (textMap.get(aiUserPreferences, userId)) {
      case null { Debug.trap("AI user preferences not found") };
      case (?_) {
        aiUserPreferences := textMap.put(aiUserPreferences, userId, preferences);
      };
    };
  ...
  // Add AI swap record
  public shared func addAISwapRecord(
    userId: Text,
    fromToken: Text,
    toToken: Text,
    amount: Float,
    ...
    let userHistory = switch (textMap.get(aiSwapHistory, userId)) {
      case null { [] };
      case (?history) { history };
    };

    aiSwapHistory := textMap.put(aiSwapHistory, userId, Array.append(userHistory, [record]));
    nextAISwapId += 1;
  };

  // Get AI swap history
  public query func getAISwapHistory(userId: Text): async [AISwapRecord] {
    switch (textMap.get(aiSwapHistory, userId)) {
      case null { [] };
      case (?history) { history };
    };
  };

  // AI-powered portfolio analysis and auto-swap logic
  public shared func analyzePortfolioAndSwap(
    userId: Text,
    currentPortfolio: [PortfolioAsset],
    marketData: [Currency]
  ): async AIAnalysisResult {
    // Get user preferences
    let preferences = switch (await getAIUserPreferences(userId)) {
      case null { 
    ...
    if (not preferences.autoHedgingEnabled) {
      return {
        shouldSwap = false;
        reason = "Auto-hedging is disabled in preferences";
    ...
    // Analyze portfolio for high-risk assets
    let highRiskAssets = Array.filter<PortfolioAsset>(
      currentPortfolio,
      func(asset) {
        asset.change24h < -Float.fromInt(preferences.thresholdPercentage)
      }
    );

    // Calculate risk score based on portfolio volatility
    let totalRisk = Array.foldLeft<PortfolioAsset, Float>(
      currentPortfolio,
      0.0,
      func(acc, asset) {
        acc + Float.abs(asset.change24h) * asset.percentage
      }
    );

    let riskScore = Int.abs(Float.toInt(totalRisk * 10.0));
    let riskScoreBounded = if (riskScore > 100) { 100 } else { riskScore };

    // Determine market conditions
    let marketCondition = if (totalRisk > 20.0) {
      "High volatility detected"
    } else if (totalRisk > 10.0) {
      "Moderate volatility"
    } else {
      "Low volatility"
    };
    ...
    let portfolioHealth = if (Array.size(highRiskAssets) > 0) {
      "At risk - consider hedging"
    } else if (riskScoreBounded > 70) {
      "High risk - monitor closely"
    } else if (riskScoreBounded > 40) {
      "Moderate risk"
    } else {
      "Healthy"
    };
    ...
    let shouldSwap = Array.size(highRiskAssets) > 0 and riskScoreBounded > 50;
    let reason = if (shouldSwap) {
      "High risk detected! " # 
      Array.foldLeft<PortfolioAsset, Text>(
        highRiskAssets,
        "",
        func(acc, asset) {
          if (acc == "") { asset.symbol } else { acc # ", " # asset.symbol }
        }
      ) # " dropped significantly. Consider hedging to " # preferences.stablecoinPreference # "."
    } else {
      "No immediate action needed. Portfolio is within acceptable risk parameters."
    };

    let recommendedAction = if (shouldSwap) {
      "Swap " # 
      Array.foldLeft<PortfolioAsset, Text>(
        highRiskAssets,
        "",
      ...
      ) # " to " # preferences.stablecoinPreference
    } else {
      "Hold current portfolio"
    };

    let suggestedTokens = if (shouldSwap) {
      [preferences.stablecoinPreference]
    } else {
      []
    };

    let confidenceLevel = if (Array.size(highRiskAssets) > 0) {
      85 // High confidence when clear risk is detected
    } else if (riskScoreBounded > 70) {
      70 // Medium confidence for high risk
    } else {
      60 // Lower confidence for normal conditions
    };
    ...
    aiAnalysisCache := textMap.put(aiAnalysisCache, userId, analysisResult);

    analysisResult;
  };

  // Get cached AI analysis
  public query func getCachedAIAnalysis(userId: Text): async ?AIAnalysisResult {
    textMap.get(aiAnalysisCache, userId);
  };

  // Get all AI user IDs
  public query func getAllAIUserIds(): async [Text] {
    Iter.toArray(textMap.keys(aiUserPreferences));
  };
