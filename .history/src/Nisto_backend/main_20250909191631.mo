...
persistent actor Nisto {
 ...
 // ========== PLUGIN SYSTEM HELPER FUNCTIONS ==========
  ...
  public func getAllCurrencies(): async [Currency] {
    // Fetch ALL currencies from real APIs - NO HARDCODED DATA
    
    // 1. Get ICP price from ICP native API
    let icpPrice = await getICPNativeRate();
    
    // 2. Get major cryptocurrencies from CoinGecko API
    ...
    switch (await makeHttpRequest(url)) {
      case (#ok(responseText)) {
        // Parse the API response and add ICP
        let apiCurrencies = parseCoinGeckoResponse(responseText);
        
        // Add ICP as the first currency with real price
        let icpCurrency: Currency = {
          id = "internet-computer";
          symbol = "ICP";
          name = "Internet Computer";
          price = icpPrice;
        ...
        // Combine ICP with API currencies
        let allCurrencies = Buffer.Buffer<Currency>(0);
        allCurrencies.add(icpCurrency);
        
        for (currency in apiCurrencies.vals()) {
          allCurrencies.add(currency);
        };
        
        Buffer.toArray(allCurrencies);
      };
      case (#err(_)) {
