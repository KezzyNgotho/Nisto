  ...
  public shared(msg) func loginOrCreateUser(): async Result.Result<User, Text> {
    ...
    switch (users.get(caller)) {
      ...
      case null { 
        ...
        let colors: [Text] = ["red","blue","green","purple","orange","silver","gold","crimson","teal","indigo","amber","violet"]; 
        let nouns: [Text] = ["sunset","river","forest","mountain","breeze","comet","ember","harbor","meadow","summit","valley","lagoon"]; 
        // Very simple deterministic seeds from principal text to avoid entropy
        let hash = Text.hash(principalText);
        let aIdx = Nat32.toNat(hash % Nat32.fromNat(animals.size()));
        let cIdx = Nat32.toNat((hash / 7) % Nat32.fromNat(colors.size()));
        let nIdx = Nat32.toNat((hash / 13) % Nat32.fromNat(nouns.size()));
        let num = Nat32.toNat((hash / 17) % 9000) + 1000; // 1000-9999
        let username = animals[aIdx] # "_" # nouns[nIdx] # "_" # Nat.toText(num);
        let displayName = Text.toUpper(Text.substr(username, 0, 1)) # Text.substr(username, 1, Text.size(username) - 1);
