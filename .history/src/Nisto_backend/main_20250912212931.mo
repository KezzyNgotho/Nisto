...
persistent actor Nisto {
  ...
  public type SmartWalletUserPreferences = {
  ...
  var userPreferences : OrderedMap.Map<Text, SmartWalletUserPreferences> = textMap.empty<SmartWalletUserPreferences>();
  ...
  // Simple file reference storage
  var fileReferences : OrderedMap.Map<Text, {path: Text; hash: Text}> = textMap.empty<{path: Text; hash: Text}>();
 ...
 // ========== PLUGIN SYSTEM HELPER FUNCTIONS ==========
  ...
  public shared func saveUserPreferences(userId : Text, preferences : SmartWalletUserPreferences) : async () {
    userPreferences := textMap.put(userPreferences, userId, preferences);
  };

  public query func getUserPreferences(userId : Text) : async ?SmartWalletUserPreferences {
  ...
  public shared func updateUserPreferences(userId : Text, preferences : SmartWalletUserPreferences) : async () {
  ...
  public shared func registerFileReference(path : Text, hash : Text) : async () {
    let fileRef = {path = path; hash = hash};
    fileReferences := textMap.put(fileReferences, path, fileRef);
  };

  public query func getFileReference(path : Text) : async {path: Text; hash: Text} {
    switch (textMap.get(fileReferences, path)) {
      case null { Debug.trap("File reference not found") };
      case (?fileRef) { fileRef };
    };
  };

  public query func listFileReferences() : async [{path: Text; hash: Text}] {
    Iter.toArray(textMap.vals(fileReferences));
  };

  public shared func dropFileReference(path : Text) : async () {
    let (newRefs, removed) = textMap.remove(fileReferences, path);
    switch (removed) {
      case null { Debug.trap("File reference not found") };
      case (?_) { fileReferences := newRefs };
    };
  };
  ...
  // Parse simple price response from CoinGecko
  private func parseSimplePriceResponse(jsonText: Text): Result.Result<[Currency], Text> {
    // For now, return a comprehensive list based on the API call
    // TODO: Implement proper JSON parsing for the simple price endpoint
    let currencies: [Currency] = [
      {
        id = "bitcoin";
        symbol = "BTC";
  ...
  public func executeSwap(request: SwapRequest): async SwapResult {
    ...
    // Store swap in history (using a different structure for history)
    let historyEntry = {
      id = swapId;
    ...
    switch (userSwapHistory.get(request.userPrincipal)) {
      case null {
        userSwapHistory.put(request.userPrincipal, [swapId]);
      };
      case (?existingSwaps) {
        let updatedSwaps = Array.append(existingSwaps, [swapId]);
        userSwapHistory.put(request.userPrincipal, updatedSwaps);
      };
    };
  ...
  public func getUserSwapHistory(user: Principal): async [SwapResult] {
    switch (userSwapHistory.get(user)) {
      case null { [] };
      case (?swapIds) {
        let results = Buffer.Buffer<SwapResult>(0);
        for (swapId in swapIds.vals()) {
          switch (swapHistory.get(swapId)) {
            case null { };
            case (?historyEntry) { 
              // Convert history entry to SwapResult
              let swapResult: SwapResult = {
                success = true;
                transactionId = ?historyEntry.id;
                fromAmount = historyEntry.inputAmount;
                toAmount = historyEntry.outputAmount;
                rate = historyEntry.exchangeRate;
                gasFee = historyEntry.gasFee;
                priceImpact = historyEntry.slippage;
                minimumReceived = historyEntry.outputAmount * (1.0 - historyEntry.slippage);
  ...
  public func getAllSwapHistory(): async [SwapResult] {
    let results = Buffer.Buffer<SwapResult>(0);
    for ((_, historyEntry) in swapHistory.entries()) {
      let swapResult: SwapResult = {
        success = true;
        transactionId = ?historyEntry.id;
        fromAmount = historyEntry.inputAmount;
