...
actor SwapService {
  ...
  // Mock API functions (will be replaced with real HTTP calls in production)
  private func fetchCryptocurrencies(): async Result.Result<[Currency], Text> {
    // Simulate API call delay
    let currencies = await getMockCurrencies();
    #ok(currencies);
  };

  // Fetch specific currency price with caching
  private func fetchCurrencyPrice(currencyId: Text): async Result.Result<Float, Text> {
    ...
    // Simulate API call and get mock price
