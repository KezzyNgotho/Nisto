...
persistent actor MPCService {
  ...
  public query func getMPCTransactions(walletId: MPCWalletId): async [MPCTransaction] {
    var result: [MPCTransaction] = [];
    for ((_, transaction) in mpcTransactions.entries()) {
      if (transaction.walletId == walletId) {
        result := Array.append(result, [transaction]);
      };
    };
    result;
  };
  ...
  public query func getSocialTransfers(userId: UserId): async [SocialTransfer] {
    var result: [SocialTransfer] = [];
    for ((_, transfer) in socialTransfers.entries()) {
      if (transfer.from == userId or transfer.to == userId) {
        result := Array.append(result, [transfer]);
      };
    };
    result;
  };
